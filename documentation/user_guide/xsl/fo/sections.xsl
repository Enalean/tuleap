...
<xsl:template name="section.heading">
  ...
  <xsl:variable name="fsize">
    <xsl:choose>
      <xsl:when test="$level=1">14</xsl:when>
  ...
  <!-- CX-SBT: For section 2, draw a line after the title -->
  <xsl:variable name="border">
    <xsl:choose>
      <xsl:when test="$level=2">solid</xsl:when>
      <xsl:otherwise>none</xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  
  <!-- CX-SBT: For section 4, use the underline decoration -->
  ...
  <fo:block text-decoration="{$decoration}" border-after-color="black" border-after-width="0.1em" border-after-style="{$border}" margin-left="0pc">
