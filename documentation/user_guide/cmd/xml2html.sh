...
${JAVA_HOME}/bin/java -cp ${CP} com.icl.saxon.StyleSheet ${PREV_DIR}/$1 ${DOC_HOME}/user_guide/xsl/htmlhelp/htmlhelp_$3.xsl > /dev/null
if [ $? != 0 ]
then 
	cd ${PREV_DIR}
        echo "Failed!"
...
${JAVA_HOME}/bin/java -cp ${CP} com.icl.saxon.StyleSheet ${PREV_DIR}/$1 ${DOC_HOME}/user_guide/xsl/htmlhelp/htmlhelp_onechunk_$3.xsl > /dev/null
if [ $? != 0 ]
then 
	cd ${PREV_DIR}
        echo "Failed!"
