...
class SVNUpdate {
	...
	/**
	 * @const 
	 * can be changed to /usr/bin/svn or any other if you want to use a specific svn bin
	 */
	const SVN_BIN = 'svn';
	
	/**
	 * @const
	 * XML tag for commits returned by the svn log command 
	 */
	const XML_COMMITS = "log";
	/**
	 * @const
	 * XML tag for commit returned by the svn log command 
	 */
	const XML_COMMIT = "logentry";
	/**
	 * @const
	 * XML attribute for commit revision returned by the svn log command 
	 */
	const XML_COMMIT_REVISION = "revision";
	/**
	 * @const
	 * XML tag for commit author returned by the svn log command 
	 */
	const XML_COMMIT_AUTHOR = "author";
	/**
	 * @const
	 * XML tag for commit date returned by the svn log command 
	 */
	const XML_COMMIT_DATE = "date";
	/**
	 * @const
	 * XML tag for commit paths returned by the svn log command 
	 */
	const XML_COMMIT_PATHS = "paths";
	/**
	 * @const
	 * XML tag for commit path returned by the svn log command 
	 */
	const XML_COMMIT_PATH = "path";
	/**
	 * @const
	 * XML attribute for commit action path returned by the svn log command 
	 */
	const XML_COMMIT_ACTION_PATH = "action";
	/**
	 * @const
	 * XML attribute for commit message returned by the svn log command 
	 */
	const XML_COMMIT_MESSAGE = "msg";
    ...
    function testUpdate($revision) {
        $simulation = $this->_SVNCommand("cd ".$this->getWorkingCopyDirectory()." ; " . self::SVN_BIN . " merge --dry-run -r ".$this->getWorkingCopyRevision().":".$revision." ".$this->getRepository());
        return $simulation;
    }
     ...
     * 
     * Use --accept=postpone option in order to avoid interactive mode in case of conflict.
     *
     * @param int $revision the revision up to we want update
     * @return string the output generated by the svn command
     */
    function updateServer($revision) {
        $update = $this->_SVNCommand("cd ".$this->getWorkingCopyDirectory()."; " . self::SVN_BIN . " update --accept=postpone -r ".$revision);
        return $update;
    }
...
// </log>
    ...
    function startElement($parser, $element_name, $attributes) {
        ...
        switch ($element_name) {
            case self::XML_COMMITS:
                break;
            case self::XML_COMMIT:
                $current_commit = new SVNCommit();
                $current_commit->setRevision($attributes[self::XML_COMMIT_REVISION]);
                break;
            case self::XML_COMMIT_AUTHOR:
                break;
            case self::XML_COMMIT_DATE:
                break;
            case self::XML_COMMIT_PATHS:
                $current_files = array();
                break;
            case self::XML_COMMIT_PATH:
                $current_file = new SVNCommitedFile();
                $current_file->setAction($attributes[self::XML_COMMIT_ACTION_PATH]);
                break;
            case self::XML_COMMIT_MESSAGE:
                $current_message = "";
                break;
            default:
                break;
    ...
    function _setCommitsFromXML($xml_commits) {
        ...
        if (!xml_parse($xml_parser, $xml_commits)) {
            trigger_error(sprintf("Unable to read the svn log: XML error: %s at line %d", xml_error_string(xml_get_error_code($xml_parser)), xml_get_current_line_number($xml_parser)), E_USER_WARNING);
        }
     ...
     * 
     * This class is based on the 1.6 SVN version, and the svn info command returns something like this :
     * Path: .
     ...
     *
     * Warning : this function is based on the subversion 1.6 version
    ...
    function _getFilesUnderSVNControl($path) {
        $files = array();
        $ls_return = $this->_SVNCommand( self::SVN_BIN . " list ".realpath($path));
        $files = explode("\n", $ls_return);
    ...
    function getConflictedLines($merge_output) {
        ...
        foreach ($lines as $line) {
            if (trim($line) != "") {
            	if ( ! $in_summary && substr($line, 0, 21) != "Summary of conflicts:") {
	                switch ($line[0]) {
	                    case "A":
	                    case "D":
	                    case "U":
	                    ...
	                    case "C":
	                    	...
	                    	break;
	                    case " ":
	                    	if ($line[1] == 'C' || $line[3] == 'C') {
	                    		$conflited_lines[] = $line;
	                    	}
	                    	break;
	                    case "-":
	                    	if (substr($line, 0, 11) == "--- Merging") {
	                    		// no error
	                		} else {
	                			$conflited_lines[] = $line;
	                		}
