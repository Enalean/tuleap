 ...
 * Version 0.9rc3 by Daniel Madruga branched from 0.9rc2 provided by Centova Technologies Inc.
 * 
 * Original Copyright 2002-2007, Centova Technologies Inc.
 * http://www.centova.com
 *
 * Portions Copyright 2002, Carlo Zottmann
 *
 ******************************************************************************
 * The version 0.9rc2 provided by Centova Technologies Inc. under GPL license
 * was modified by Daniel Madruga in order to introduce support for:
 * - generic_iq event, which handles generic iq packets and calls a user defined
 *   method with the packet as parameter. 
 * - muc_join event, which handles ack msg after joining a MUC room.
 * - Added special handler for connection time-out.
 * - Properly handle messages comming from JID which are not present in the user's roster 
 ...
 * HISTORY
 *  v0.9rc3 (Apr 14, 2008)
 * 		 - Support for generic iq packets.
 * 		 - Support for MUC room creation ack msgs.
 *
 *  v0.9rc2 (Oct 15, 2007)
...
added by Daniel Madruga
$this->_call_handler("generic_id",$packet);
$this->_call_handler('muc_join',$to,$from,$jid,$affiliation,$role,$status);
$this->_call_handler("timeout");
...
define("MIN_CALLBACK_FREQ",0.3);	// more than once per second is dangerous
define("MAX_CALLBACK_FREQ",1); // less than once every 10 seconds will be very, very slow

// Log file
if(!defined('JABBER_LOG_FILE')){
...
class Jabber {
	...
	var $_iq_version_name	= "class_Jabber.php";
	...
	var $_log_flush			= false;

	// Log file
	var $_log_file_path = JABBER_LOG_FILE;
	...
	var $auto_server_identify = false;
	...
	function _log($msg,$level = 1) {
		if ($this->_use_log) {
			if ($this->_log_flush) $this->_log_file = @fopen($this->_log_file_path,"a");
			
			if ($this->_log_file) {
				fwrite($this->_log_file,microtime(true)."- $msg\n");
	...
	function _log_open() {
		if ($this->_use_log) {
			$log_file = $this->_log_file_path;
			
			// Keep the log file < 500K
			if( file_exists($log_file) && (($fsize=filesize($log_file)) > 512000) ){ // Check whether the log file is > 500K
				$content = file_get_contents($log_file,NULL,NULL,$fsize-256000); // Get the last 250K
				$fh = fopen($log_file, 'w');
				fwrite($fh,$content);
				fclose($fh);
			}
...
//		$this->_set_iq_handler("_on_xxx_result",$data_id); // we don't really need the result from this... do we?
	...
	// Modified by Daniel Madruga to introduce a special handler for time-out.
	function execute($callback_freq = -1,$seconds = -1)
	{
		...
		if ($this->execute_loop && ($count == $seconds) ){
			$this->_call_handler("timeout");
		}
		else if($this->execute_loop && $this->terminated){
			$this->_call_handler("terminated");
		}
	...
	function _handle_message(&$packet) {
		...
		switch($type) {
			...
			case "normal":
			default:
				if ( isset($this->roster[$from_jid]) && $this->roster[$from_jid]["composing"] ) $this->roster[$from_jid]["composing"] = false;
	...
	function _handle_presence(&$packet) {	
		...
		/*
		 * Added by Daniel Madruga.
		 * Support for MUC rooms creation.
		 */
		// Check if this presence pck carries the XEP http://jabber.org/protocol/muc#user
		// mark on it, and if so call the method associated to muc_join.
		$xep = $this->_node($packet,array('presence','#','x','0','@','xmlns'));
		if( !strcmp($xep,"http://jabber.org/protocol/muc#user") || !strcmp($xep,"http://jabber.org/protocol/muc") ){
			$to = $this->_node($packet,array('presence','@','to'));
			$jid = $this->_node($packet,array('presence','#','x','0','#','item','0','@','jid'));
			$affiliation = $this->_node($packet,array('presence','#','x','0','#','item','0','@','affiliation'));
			$role = $this->_node($packet,array('presence','#','x','0','#','item','0','@','role'));
			$status = $this->_node($packet,array('presence','#','x','0','#','status','0','@','code'));
			$this->_call_handler('muc_join',$to,$from,$jid,$affiliation,$role,$status,$type);
			return true;
		}
	...
	/*
	 * Added by Daniel Madruga (dmadruga at gmail dot com)
	 * Used to handler a generic iq (unknown by any other handler).
	 * This method just calls a handling method defined by the user with _set_iq_handler
	 * passing the packet as parameter.
	 * 
	 */
	function on_generic_iq(&$packet){
		$this->_call_handler("generic_iq",$packet);
	}
