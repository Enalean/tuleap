...
build:
    ...
    cd /build/src/plugins/botmattermost && pnpm --filter '@tuleap/plugin-botmattermost...' --workspace-concurrency=-2 run build && \
	composer install --classmap-authoritative --no-dev --no-interaction --no-scripts
