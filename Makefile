...
PKG_NAME=tuleap-plugin-botmattermost
...
build:
    ...
    cd /build/src && pnpm run build -- --scope=@tuleap/plugin-botmattermost --include-dependencies && \
	cd /build/src/plugins/botmattermost && composer install --classmap-authoritative --no-dev --no-interaction --no-scripts
...
$(RPM_TMP)/SOURCES/$(NAME_VERSION).tar.gz: build $(RPM_TMP)
	...
	cp -ar $(BASE_DIR)/../../src/www/assets/botmattermost $(RPM_TMP)/SOURCES/$(NAME_VERSION)/assets
	cd $(RPM_TMP)/SOURCES && \
		find $(NAME_VERSION)/ \(\
...
docker-run:
	...
	cp -Rf /plugin/ /build/src/plugins/botmattermost
	make -C /build/src/plugins/botmattermost all RELEASE=$(RELEASE) DIST=.$(DIST)
