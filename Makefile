...
docker-run:
	...
	cp -Rf /plugin/ /build/src/plugins/botmattermost && chown -R build /build/src
	su --login --command "make -C /build/src/plugins/botmattermost all RELEASE=$(RELEASE) DIST=.$(DIST)" build
