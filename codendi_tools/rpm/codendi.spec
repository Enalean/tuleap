...
%define APP_DIR %{_datadir}/codendi
%define APP_LIB_DIR /usr/lib/codendi
%define APP_LIBBIN_DIR %{APP_LIB_DIR}/bin
%define APP_DATA_DIR %{_localstatedir}/lib/codendi
%define APP_CACHE_DIR %{_localstatedir}/tmp/codendi
...
%{__install} -m 755 codendi_tools/setup.sh $RPM_BUILD_ROOT/%{_datadir}/tuleap-install/setup.sh
#
...
# /usr/share
(cd $RPM_BUILD_ROOT%{_datadir} && [ -e tuleap ] || %{__ln_s} codendi tuleap)
# /etc
(cd $RPM_BUILD_ROOT%{_sysconfdir} && [ -e tuleap ] || %{__ln_s} codendi tuleap)
# /usr/lib
(cd $RPM_BUILD_ROOT/usr/lib && [ -e tuleap ] || %{__ln_s} codendi tuleap)
# /var/lib
(cd $RPM_BUILD_ROOT%{_localstatedir}/lib && [ -e tuleap ] || %{__ln_s} codendi tuleap)
# /var/tmp
#(cd  $RPM_BUILD_ROOT%{_localstatedir}/tmp && [ -e tuleap ] || %{__ln_s} codendi_cache tuleap)
# /var/log
#(cd $RPM_BUILD_ROOT%{_localstatedir}/log && [ -e tuleap ] || %{__ln_s} codendi tuleap)

# Backward compatability
(cd $RPM_BUILD_ROOT/etc/rc.d/init.d && [ -e 'codendi' ] || %{__ln_s} %{APP_NAME} codendi)
#%{__ln_s} $RPM_BUILD_ROOT/etc/rc.d/init.d/%{APP_NAME} $RPM_BUILD_ROOT/etc/rc.d/init.d/codendi
...
%dir /etc/rc.d/init.d/codendi
%dir %{_datadir}/tuleap
%dir %{_sysconfdir}/tuleap
%dir /usr/lib/tuleap
%dir %{_localstatedir}/lib/tuleap
#%dir %{_localstatedir}/tmp
#%dir %{_localstatedir}/log/tuleap
