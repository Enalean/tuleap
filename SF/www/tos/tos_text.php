...
<p>
<h2><?php print $GLOBALS['sys_name']; ?> Terms of Service Agreement</h2>
</p>
...
<p>
A person's use of <?php print $GLOBALS['sys_name']; ?> and any of its Services (as defined below) at
any time is subject to <?php print $GLOBALS['sys_name']; ?> then-current Terms of Service ("TOS"),
which may be updated from time to time as set forth below. Each user can
review the then-current TOS at any time at:
<a href="/tos/tos.php">http://<?php print $GLOBALS['sys_default_domain']; ?>/tos/tos.php</a>.  In addition, each user's use of
...
reference into the TOS.  A user's use of  <?php print $GLOBALS['sys_name']; ?> and/or its Services
constitutes full acceptance of and agreement to the TOS; if a user does not
accept our TOS, he or she is not granted rights to use <?php print $GLOBALS['sys_name']; ?> or any of
its Services, as defined herein, and should refrain from accessing
<?php print $GLOBALS['sys_name']; ?> and its Services.
</p>
...
<p>
<?php print $GLOBALS['sys_name']; ?> reserves the right at any time and from time to time to modify
or discontinue, temporarily or permanently, <?php print $GLOBALS['sys_name']; ?> or any Service
...
<?php print $GLOBALS['sys_name']; ?> shall not be liable to any user or other third party for any
...
thereunder.    <?php print $GLOBALS['sys_default_domain']; ?> cannot and will not be liable for any loss or
...
<?php print $GLOBALS['sys_name']; ?> of any unauthorized use of his or her password or account or any
...
that the user, and not the <?php print $GLOBALS['sys_name']; ?> team, is entirely responsible for all Content
...
<p>
<?php print $GLOBALS['sys_name']; ?> will not pre-screen or review Content, but <?php print $GLOBALS['sys_name']; ?> reserves
the right to refuse or delete any Content of which it becomes aware which it
reasonably deems not to fulfill the Purpose. In addition, <?php print $GLOBALS['sys_name']; ?> shall
...
otherwise illegal.  <!-- LJ <?php print $GLOBALS['sys_name']; ?>, in its sole and absolute discretion, may
...
us to protect the rights, property or personal safety of <?php print $GLOBALS['sys_name']; ?>, its
users and the public -->. <?php print $GLOBALS['sys_name']; ?> does not control the Content posted via
the Service and, as such, does not guarantee the accuracy, integrity or
quality of such Content. Under no circumstances will <?php print $GLOBALS['sys_name']; ?> be liable
...
transmitted via <?php print $GLOBALS['sys_name']; ?> or any Service thereon.
</p>


<p>
<B>5. GENERAL PRACTICES REGARDING USE AND STORAGE</B> 
</p>

<p>
<?php print $GLOBALS['sys_name']; ?> may establish general practices and limits concerning use of the
Services, as defined by posted Service-specific Rules.  While <?php print $GLOBALS['sys_name']; ?>
will use reasonable efforts to <a href="/docman/display_doc.php?docid=17&group_id=1#whyhost-backup" target="side_window">backup</a> site data and make such data
available in the event of loss or deletion, <?php print $GLOBALS['sys_name']; ?> has no
...
by any Service. <?php print $GLOBALS['sys_name']; ?> reserves the right to mark as "inactive" and
archive accounts and/or projects that are inactive for an extended period of
time. <?php print $GLOBALS['sys_name']; ?> reserves the right to change these general practices and
...
<p>
Termination of any user's access to <?php print $GLOBALS['sys_name']; ?> or any Service
hereunder may be effected without notice and, on such termination,
<?php print $GLOBALS['sys_name']; ?> may immediately deactivate or delete user's account and/or bar
any further access to such files, that Service or <?php print $GLOBALS['sys_name']; ?>.  <?php print $GLOBALS['sys_name']; ?>
