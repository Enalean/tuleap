...
function filterlist(selectobj) {
  ...
  this.addOption = function(new_option) {
    ...
    this.optionscopy[this.optionscopy.length]=new Option(new_option.text, new_option.value, false);
  }
  ...
  this.set = function(pattern) {
    ...
    for (loop=0; loop < this.optionscopy.length; loop++) {
      ...
      return this.debug('object #'+(loop)+" not defined, previous obj="+this.selectobj.options[index-2].text);
    }
...
function selIt(btn) {
  ...
  for (var i = 0; i < pickOLength; i++) {
    if (pickOptions[i].text != noSelectText)
