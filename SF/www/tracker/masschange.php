...
while (list($field,$value_id) = each($prefs)) {
    ...
    if ( ($field_object)&&($field_object->isDateField()) ) {
		if ($advsrch) {
		    ...
		    //echo 'DBG Setting $prefs['.$field.'_end]= '.$prefs[$field.'_end'].', \$\$field_end = $$field_end<br>';
		} else {
		    $field_op = $field.'_op';
		    $prefs[$field_op] = $$field_op;
		    if (!$prefs[$field_op])
				$prefs[$field_op] = '>';
		    //echo 'DBG Setting $prefs['.$field.'_op]= '.$prefs[$field.'_op'].', \$\$field_op = $$field_op<br>';
		}
    }
}
...
$params=array('title'=>$group->getPublicName().': \''.$ath->getName().'\' '.$Language->getText('tracker_masschange','mass_change_report'),
