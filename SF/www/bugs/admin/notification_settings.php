...
$res_roles = bug_data_get_notification_roles();
...
$res_events = bug_data_get_notification_events();
...
$res_notif = bug_data_get_notification($user_id);
...
if ($submit) {
	...
	if ($res_watch) {
	    bug_data_delete_watchees($user_id); 
	    $res_watch = bug_data_insert_watchees($user_id, array_keys($arr_user_ids));
	}   
    ...
    bug_data_delete_notification($user_id);
    $res_notif = bug_data_insert_notification($user_id, $arr_roles, $arr_events, $arr_notif);
...
// Get group information bur new bug notification settings
...
// Build Wachees UI
$res = bug_data_get_watchees($user_id);
$arr_watchees = array();
...
notification of new bug submission (and possibly updates).<P><u>Recipients</u>: '.
	    $row_grp['new_bug_address'].'
	&nbsp;&nbsp;&nbsp; (Send on all updates: '.(($row_grp['send_all_bugs'])?'Yes':'No').')<p>';
    else
	echo '
                <P>The project administrator hasn\'t yet specified email addresses that will systematically receive
 email notification of new bugs.<P>';
}
...
';
    ...
    $res = bug_data_get_watchers($user_id);
    $arr_watchers = array();
    ...
    if ($watchers) {
	echo "<p>For your information your own bug notifications are currently watched by: <u>$watchers</u>";
    } else {
	echo "<p>For your information <u>nobody</u> is currently watching your own bug notifications ";
    }
    echo '<br><br>';
}
...
for ($j=0; $j<$num_events; $j++) {
	...
	if ( (($role_label == 'ASSIGNEE') && !$is_user_a_member) ||
	     (($event_label == 'NEW_BUG') && ($role_label != 'ASSIGNEE') && ($role_label != 'SUBMITTER')) ) {
	    // if the user is not a member then the ASSIGNEE column cannot
	    // be set. If it's not an assignee or a submitter the new_bug event is meaningless
