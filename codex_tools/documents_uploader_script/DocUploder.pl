  ...
  #                                                                     #
  # usage: DocUploader project project_id source id_dest                #
  #                                                                     #
  # where:                                                              #
  #   -project is your project shortname (where you need                # 
  #            to upload  documents)                                    #
  #   -project_id is your project ID                                    # 
  #   -source is the local path to the directory containing             #
  #           documents you need to upload                              #
  #   -id_destination is the id of the folder (in codex doc             #
  #                   manager)                                          #
  # NB: You have to specify the path to your codex cli in the line 34   #                                                                    #                                                                                                                                          #
  #                                                                     #                                                                                                      #
  ...
  if ($#ARGV != 3) {
      print "usage: DocUploader project project_id source id_destination \n";
      exit;
  }
  ...
  $_source      = $ARGV[2];
  $_id_dest     = $ARGV[3];
  $_login_url   = 'https://codex.cro.st.com/account/login.php';
  	...
  	if ($source_table[$i][1] == $j)
  	{
        ...
        $_create_folder_cmd="$_cli_dir"."/"."./codex.php docman createFolder --project="."$_project"." --parent_id="."$_parent_id"." --title=\""."$source_table[$i][0]"."\"" ;
  		...
  		else
  		{
  			...
  			###########################################################
			    ...
			    $_upload_document_url = "https://codex.cro.st.com/plugins/docman/?group_id="."$_project_id"."&action=newDocument&id="."$_id_dest"."&bc=1";  
        ...
        ###################################################
  			...
  			######################################################
			    ...
			    $_upload_document_url = "https://codex.cro.st.com/plugins/docman/?group_id="."$_project_id"."&action=newDocument&id="."$_parent_id"."&bc=1";  
