...
if ($result_docman->fetchrow()) {
    ...
    $insert->execute($root, 'Documentation en fran�ais', '', 1, 1, undef, undef);
    $id = $dbh->{q{mysql_insertid}};
    $permissions->execute($id, $id);
    $fr = $id;
    
    $insert->execute($fr, 'Guide de l\'Utilisateur CodeX', 'Un guide complet d�crivant tous les services de CodeX et comment les utiliser de mani�re optimale. Fournit �galement de nombreuses astuces et explications pour g�rer efficacement votre projet CodeX.', -1, 1, undef, undef);
    $id = $dbh->{q{mysql_insertid}};
    $permissions->execute($id, $id);
    $cug = $id;
    ...
    $insert->execute($fr, 'Interface de Commande en Ligne', 'Un guide complet d�crivant toutes les fonctions de l\'Interface de Commande en Ligne de CodeX.', 0, 1, undef, undef);
    $id = $dbh->{q{mysql_insertid}};
    $permissions->execute($id, $id);
    $cli = $id;
