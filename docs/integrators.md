...
-   Merge the patch from Gerrit (see the "download" section in Gerrit) and sign the merge commit:

    ``` bash
    $ git fetch ssh://username@gerrit.tuleap.net:29418/tuleap refs/changes/52/52/8 && \
      git merge --no-ff -S --log FETCH_HEAD
    ```
...
This is ensured by:
* The permissions set on the Tuleap repository (see [the corresponding Access Control on Git repositories in the Tuleap documentation](https://docs.tuleap.com/user-guide/code-versioning/git.html#access-control))
