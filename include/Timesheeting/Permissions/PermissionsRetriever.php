...
namespace Tuleap\Timesheeting\Permissions;
...
class PermissionsRetriever
{
    ...
    public function __construct(TimesheetingUgroupRetriever $timesheeting_ugroup_retriever)
    {
    ...
    {
        foreach ($this->timesheeting_ugroup_retriever->getWriterIdsForTracker($tracker) as $ugroup_id) {
    ...
    {
        foreach ($this->timesheeting_ugroup_retriever->getReaderIdsForTracker($tracker) as $ugroup_id) {
            if ($user->isMemberOfUGroup($ugroup_id, $tracker->getGroupId(), $tracker->getId())) {
                return true;
            }
        }
