...
class NoMoreSingletonitusTest extends TuleapTestCase {
    public function testThereAreNoNewClassFilesContainingSingletonLookups() {
        $current_number_of_singleton_lookups = 746;
        $systemOutput = $this->getSystemOutput('grep -rc  "::instance()" * | grep -v :0| grep -c ""');
        $this->assertTrue($systemOutput[0] <= $current_number_of_singleton_lookups, 
        ...
        $this->assertEqual($systemOutput[0] >= $current_number_of_singleton_lookups, 
