...
class SVNCLITest extends \Tuleap\Test\PHPUnit\TestCase
{
    ...
    public function testWriteAccessDeniedToBob()
    {
        ...
        $message       = '';
        try {
            $this->command($this->getSvnCommand('bob', 'cp -m "stuff" https://tuleap/svnplugin/svn-project-01/sample/trunk https://tuleap/svnplugin/svn-project-01/sample/branches/v1'));
        } catch (\Exception $e) {
    ...
    public function testWriteAccessGrantedToAlice()
    {
        $got_exception = false;
        try {
            //$this->command($this->getSvnCommand('alice', 'cp -m "stuff" https://tuleap/svnplugin/svn-project-01/sample/trunk https://tuleap/svnplugin/svn-project-01/sample/branches/v1'));
            $this->command($this->getSvnCommand('alice', 'rm -m "Clean" https://tuleap/svnplugin/svn-project-01/sample/branches/v1'));
        } catch (\Exception $e) {
            if (preg_match('/Access to \'.*\' forbidden/', $e->getMessage())) {
                $got_exception = true;
            }
