...
class UserGroupTest extends RestBase {
   ...
   public function testGETId() {
        ...
        $this->assertEquals(
            ...
            array(
                'id'        => '101_101',
                'uri'       => 'user_groups/101_101',
                'label'     => 'static_ugroup',
                'users_uri' => 'user_groups/101_101/users'
                )
        );
    ...
    public function testGETIdDoesNotWorkIfUserIsProjectMemberButNotProjectAdmin() {
        ...
        $exception = false;
        try {
            $response = $this->getResponse($this->client->get('user_groups/104_101'));
    ...
    public function testGETIdDoesNotWorkIfUserIsNotProjectMember() {
        ...
        $exception = false;
        try {
            $response = $this->getResponse($this->client->get('user_groups/102_101'));
        } catch (Guzzle\Http\Exception\BadResponseException $e) {
            $this->assertEquals($e->getResponse()->getStatusCode(), 403);
            $exception = true;
        }
    ...
    public function testGETIdThrowsA404IfUserGroupIdDoesNotExist() {
        ...
        $exception = false;
        try {
            $response = $this->getResponse($this->client->get('user_groups/102_999'));
        } catch (Guzzle\Http\Exception\BadResponseException $e) {
