...
final class UGroupBindingTest extends TestCase
{
    ...
    public function testResetUgroupFailureUpdateUGroupNotAllowed(): void
    {
        ...
        $this->binding->resetUgroup(200);
    }

    public function testResetUgroupDaoFailure(): void
    {
        $this->ugroup_manager->shouldReceive('isUpdateUsersAllowed')
            ->once()
            ->andReturnTrue();
    ...
    public function testResetUgroup(): void
    {
        $this->ugroup_manager->shouldReceive('isUpdateUsersAllowed')
            ->once()
            ->andReturnTrue();
    ...
    public function testCloneUgroupFailureUpdateUGroupNotAllowed(): void
    {
        $this->ugroup_manager->shouldReceive('isUpdateUsersAllowed')
            ->once()
            ->andReturnFalse();
        ...
        $this->binding->cloneUgroup(300, 200);
    }

    public function testCloneUgroupDaoFailure(): void
    {
        $this->ugroup_manager->shouldReceive('isUpdateUsersAllowed')
            ->once()
            ->andReturnTrue();
    ...
    public function testCloneUgroup(): void
    {
        $this->ugroup_manager->shouldReceive('isUpdateUsersAllowed')
            ->once()
            ->andReturnTrue();
    ...
    public function testUpdateUgroupBindingFailure(): void
    {
        $this->ugroup_manager->shouldReceive('updateUgroupBinding')
            ->once()
            ->andReturnFalse();

        $this->expectExceptionMessage('Unable to store ugroup binding');
    ...
    public function testReloadUgroupBindingResetsAndClones(): void
    {
        $this->binding = Mockery::mock(UGroupBinding::class)->makePartial();
        $this->binding->shouldReceive('resetUgroup')
            ->with(200)
            ->once();
        $this->binding->shouldReceive('cloneUgroup')
            ->with(300, 200)
            ->once();
        $this->binding->shouldReceive('getUGroupsByBindingSource')
            ->with(200)
        ...
        $this->binding->reloadUgroupBinding(200, 300);
    }

    public function testReloadUgroupBindingRecursivelyReloadsBoundUgroups(): void
    {
        $this->binding = Mockery::mock(UGroupBinding::class)->makePartial();
        $this->binding->shouldReceive('resetUgroup')->with(200)->once();
        $this->binding->shouldReceive('resetUgroup')->with(400)->once();
        $this->binding->shouldReceive('resetUgroup')->with(500)->once();
        $this->binding->shouldReceive('cloneUgroup')->with(300, 200)->once();
        $this->binding->shouldReceive('cloneUgroup')->with(200, 400)->once();
        $this->binding->shouldReceive('cloneUgroup')->with(200, 500)->once();
        $bound_ugroup = [400 => [], 500 => []];
        $this->binding->shouldReceive('getUGroupsByBindingSource')
            ->with(200)
            ->once()
            ->andReturn($bound_ugroup);
        $this->binding->shouldReceive('getUGroupsByBindingSource')
            ->with(Mockery::anyOf(400, 500))
            ->andReturn([]);

        $this->binding->reloadUgroupBinding(200, 300);
    }

    public function testReloadUgroupBindingRethrowsLogicExceptions(): void
    {
        $this->binding = Mockery::mock(UGroupBinding::class)->makePartial();
        $this->binding->shouldReceive('resetUgroup')
            ->once()
            ->andThrow(new \LogicException('Unable to reset ugroup'));

        $this->expectExceptionMessage('Unable to reset ugroup');

        $this->binding->reloadUgroupBinding(200, 300);
    }

    public function testReloadUgroupBindingAddsFeedbackWhenRuntimeException(): void
    {
        $this->binding = Mockery::mock(UGroupBinding::class)->makePartial();
        $this->binding->shouldReceive('resetUgroup');
        $this->binding->shouldReceive('cloneUgroup')
            ->andThrow(new \RuntimeException('Update users is not allowed for the ugroup'));

        $GLOBALS['Response']->shouldReceive('addFeedback')
            ->with('warning', 'Update users is not allowed for the ugroup');
