...
function logCheckingEmails(): void {
...
export function registerEmailCommands(): void {
    ...
    Cypress.Commands.add(
        ...
        (from_email_address: string, attachment_type: string): void => {
            ...
            cy.reloadUntilCondition(
                logCheckingEmails,
                conditionCallback,
                `Email address "${from_email_address}" did not send an email with an attachment`,
            );
        },
    );
    ...
    function getEmailsReceivedBy(
        ...
        return cy.request({
            method: "GET",
            url:
                "http://mailhog:8025/api/v2/search?kind=to&query=" +
