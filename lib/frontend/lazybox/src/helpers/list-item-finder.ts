...
export function getNextItem(current_item: Element): Element | null {
    ...
    while (next_item) {
        ...
        if (next_item && next_item.classList.contains("lazybox-dropdown-option-value")) {
            break;
        }
    }
    ...
    const current_group = current_item.closest(".lazybox-item-group");
    if (current_group && current_group.nextElementSibling) {
        return current_group.nextElementSibling.querySelector(".lazybox-dropdown-option-value");
    }
...
export function getPreviousItem(current_item: Element): Element | null {
    ...
    while (previous_item) {
        ...
        if (previous_item && previous_item.classList.contains("lazybox-dropdown-option-value")) {
            break;
        }
    }
