...
describe("KeyboardNavigationManager", () => {
    ...
    function assertOnlyOneItemIsHighlighted(): void {
        expect(dropdown_list.querySelectorAll(".lazybox-item-highlighted")).toHaveLength(1);
    }
    ...
    beforeEach(() => {
        ...
        content_renderer.renderLazyboxDropdownContent(groups);
    ...
    describe("arrows up/down", () => {
        ...
        describe("ArrowDown key", () => {
            it("removes the highlight on the previous item and highlights the next one", () => {
                ...
                expect(
                    item_map_manager
                        .findLazyboxItemInItemMap("lazybox-item-group1-value-1")
                        .element.classList.contains("lazybox-item-highlighted")
                ).toBe(true);
            });
            ...
            it("When the user reaches the last valid item, then it should keep it highlighted", () => {
                ...
                expect(
                    item_map_manager
                        .findLazyboxItemInItemMap("lazybox-item-group2-value-4")
                        .element.classList.contains("lazybox-item-highlighted")
                ).toBe(true);
            });
        });
        ...
        describe("ArrowUp key", () => {
            it("removes the highlight on the next item and highlights the previous one", () => {
                ...
                expect(
                    item_map_manager
                        .findLazyboxItemInItemMap("lazybox-item-group1-value-0")
                        .element.classList.contains("lazybox-item-highlighted")
                ).toBe(true);
            });
