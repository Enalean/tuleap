...
export class ItemsMapManager {
    ...
    public getLazyboxItems(): ReadonlyArray<RenderedItem> {
        return [...this.items_map.values()];
    }

    public findLazyboxItemInItemMap(item_id: string): RenderedItem {
    ...
    public refreshItemsMap(groups: GroupCollection): void {
        this.items_map = this.list_item_builder.buildLazyboxItemsMap(groups);
    }

    public getItemWithValue(value: unknown): RenderedItem | null {
        return this.getLazyboxItems().find((item) => item.value === value) ?? null;
    }
}
