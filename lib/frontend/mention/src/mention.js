...
export function initMentions(selector) {
    jQuery(selector)
        ...
        .on("inserted.atwho", (event) => {
            if (!(event.target instanceof HTMLElement)) {
                return;
            }
            // Trigger artificial input on textarea so that code depending on those events can catch the change.
            event.target.dispatchEvent(new InputEvent("input", { bubbles: true }));
        });
}
