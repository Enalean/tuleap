...
import type { HostElement } from "./subscript";
import { connect, renderSubscriptItem } from "./subscript";

describe("SubscriptElement", () => {
    ...
    it("When the component is connected, then it should set its part of the toolbar view so it will be able to update itself when the view changes.", () => {
        ...
        toolbar_bus.view.activateSubscript(true);
        expect(host.is_activated).toBe(true);

        toolbar_bus.view.activateSubscript(false);
        expect(host.is_activated).toBe(false);
    });

    it("When the button is clicked, Then it should call toolbar_bus subscript() method", () => {
        const applyCode = vi.spyOn(toolbar_bus, "subscript");
        const host = { toolbar_bus } as HostElement;

        renderSubscriptItem(host, gettext_provider)(host, target);

        const button = target.querySelector<HTMLButtonElement>("[data-test=button-subscript]");
        if (!button) {
