...
export const useDragAutoscrollWithPointerEvents = (
    global_window: Window = window,
): DragAutoscroll => getDragAutoscroll(global_window, "pointermove");
export const useDragAutoscrollWithDraggableEvents = (
    global_window: Window = window,
): DragAutoscroll => getDragAutoscroll(global_window, "dragover");

export function getDragAutoscroll(
    global_window: Window = window,
    mouse_event_name: "pointermove" | "dragover",
): DragAutoscroll {
    ...
    function start(): void {
        global_window.document.addEventListener(mouse_event_name, handlePointerMove);
        startScrollAnimation();
    }

    function stop(): void {
        global_window.document.removeEventListener(mouse_event_name, handlePointerMove);
        stopScrollAnimation();
