...
export class StatisticsPieChart {
    ...
    initLegend() {
        ...
        legend.append("span").attr("class", (d) => {
            return `${this.getLegendColorSpanClass()} ${this.getLegendColorClassByKey(d.key)} pie-chart-span-legend-color ${d.color}`;
        });
    ...
    initGraph() {
        const arc_elements = this.group
            ...
            .append("g")
            .attr("class", (d) => {
                return `${this.getSliceClass()} ${this.getSliceClassByKey(d.data.key)} ${d.data.color}`;
            });

        arc_elements
            .append("path")
            .attr("class", `${this.getSlicePathClass()} pie-chart-slice-path`)
            .attr("d", this.arc);

        arc_elements
            .append("text")
            .attr("class", `${this.getSliceTextClass()} pie-chart-slice-text`)
            .attr("transform", (d) => {
