...
<template>
    <div class="breadcrumb-privacy-icon-container">
        ...
        >
            <i
                class="fa-solid breadcrumb-project-privacy-icon"
                v-bind:class="project_privacy_icon"
            ></i>

            <span
                class="current-project-nav-flag-labels"
                v-if="has_project_flags"
                data-test="project-flags"
            >
                <span
...
const popover_icon = ref<HTMLElement | undefined>();
const popover_content = ref<HTMLElement | undefined>();
...
const has_project_flags = computed((): boolean => {
    return props.project_flags.length > 0;
});
const project_privacy_icon = computed((): string => {
    return getProjectPrivacyIcon(props.privacy);
});
