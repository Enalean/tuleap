...
export class DropdownManager {
    ...
    constructor(
        ...
        const resize_dropdown_callback = (entries: readonly ResizeObserverEntry[]): void => {
            ...
            if (
                entries.length === 1 &&
                entries[0].target.classList.contains("link-selector-dropdown")
            ) {
    ...
    public isDropdownOpen(): boolean {
        return this.dropdown_element.classList.contains("link-selector-dropdown-shown");
    }

    public closeLinkSelector(): void {
        if (!this.isDropdownOpen()) {
            return;
        }
