...
echo "machine nix-cache.enalean.com" > ~/.config/nix/netrc-nix-cache-enalean
echo "password ${AUTH_TOKEN}" >> ~/.config/nix/netrc-nix-cache-enalean
...
echo 'default-server = "enalean"' > ~/.config/attic/attic.toml
echo '[servers.enalean]' >> ~/.config/attic/attic.toml
echo 'endpoint = "https://nix-cache.enalean.com"' >> ~/.config/attic/attic.toml
echo "token = \"${AUTH_TOKEN}\"" >> ~/.config/attic/attic.toml
