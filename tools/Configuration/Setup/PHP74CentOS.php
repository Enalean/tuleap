...
class PHP74CentOS
{
    ...
    public function main(): void
    {
        try {
            ...
            $this->logger->info('Configure Tuleap for PHP 7.4 / FPM and Nginx');
    ...
    private function configure(array $modules, $for_development): void
    {
        ...
        if (in_array('fpm', $modules, true)) {
            $configs[] = SiteDeployFPM::buildForPHP74($this->logger, $variables->getApplicationUser(), $for_development);
        }
        if (in_array('nginx', $modules, true)) {
            $configs[] = new SiteDeployNginx(
                new Wrapper($this->logger, 'nginx'),
    ...
    {
        echo <<<EOT
Usage: /usr/share/tuleap/tools/utils/php74/run.php [--module=nginx,fpm]

Configuration of Tuleap for usage of PHP 7.4 / FPM and Nginx
