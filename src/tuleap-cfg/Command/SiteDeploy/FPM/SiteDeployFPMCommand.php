...
final class SiteDeployFPMCommand extends Command
{
    ...
    protected function configure(): void
    {
        $this->setDescription('Deploy PHP FPM configuration files')
            ->addOption(self::OPT_PHP_VERSION, '', InputOption::VALUE_REQUIRED, 'Target php version: `php80` (default), `php81`', self::PHP80)
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($php_version === self::PHP80) {
            $deploy = SiteDeployFPM::buildForPHP80($console_logger, ForgeConfig::get('sys_http_user'), $development);
        } elseif ($php_version === self::PHP81) {
