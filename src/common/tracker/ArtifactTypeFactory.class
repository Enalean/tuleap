...
class ArtifactTypeFactory extends Error {
	...
	function create($group_id,$group_id_template,$atid_template,$name,$description,$itemname,$ugroup_mapping=false) {
		...
		$sql="INSERT INTO 
			artifact_group_list 
			(group_id, name, description, item_name, allow_copy,email_all_updates,email_address,submit_instructions,browse_instructions
                         ) 
			VALUES 
			('". $group_id ."',
			'". $name ."',
			...
			'". $itemname."',
                        ...
                        '". db_escape_string($at_template->getBrowseInstructions())."')";
		//echo $sql;
		$res = db_query($sql);
