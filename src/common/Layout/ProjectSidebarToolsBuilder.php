    ...
    public function getSidebarTools(PFUser $user, $toptab, ?string $active_promoted_item_id, Project $project): \Generator
    {
        ...
        foreach ($project->getServices() as $service) {
            ...
            if ($service instanceof ProjectDefinedService) {
                yield SidebarServicePresenter::fromProjectDefinedService($service, $href, $this->isEnabled($toptab, $service), $active_promoted_item_id, $user);
            } else {
    ...
    private function getLink(\Service $service, Project $project): string
    {
        ...
        if ($project_id == 100) {
            if (strpos($link, '$projectname') !== false) {
                ...
                // * src/common/Project/RegisterProjectStep_Confirmation.php
