...
It has no built-in <code>&lt;select&gt;</code> element, it is not linked to <code>&lt;form&gt;</code> validation / submit. If you need that, please use <a href="/?path=/docs/tlp-forms-list-picker--docs">List picker</a> or add the values to a hidden <code >&lt;input&gt;</code> by yourself.
