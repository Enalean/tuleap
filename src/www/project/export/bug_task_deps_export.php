...
"$group_id AS group_id,bug_task_dependencies.is_dependent_on_task_id ".
'FROM bug_task_dependencies, bug '.
'WHERE bug_task_dependencies.bug_id=bug.bug_id AND '.
"bug.group_id='$group_id' AND ".
'bug_task_dependencies.is_dependent_on_task_id <> 100';

$col_list = array('bug_id','group_id','is_dependent_on_task_id');

$lbl_list = array('bug_id' => 'Bug ID',
	     'group_id' => 'Group ID',
	     'is_dependent_on_task_id' => 'Depend on bug');
$dsc_list = array('bug_id' => 'Unique bug identifier',
	     'group_id' => 'Unique project identifier',
	     'is_dependent_on_task_id' => 'Task  the project bug depends on');

$tbl_name = 'bug_task_dependencies';
...
if ($export == 'bug_task_deps') {
	...
	if ($result) {
	    echo '<P>No bug-task depencies  found. Could not generate an export.';
	} else {
	    echo '<P>Error while accessing your bug depencies database. Please report the error to the '.$GLOBALS['sys_name'].' Administrator';
	    echo '<br>'.db_error();
	}
...
} else if ($export == "bug_task_deps_format") {
    
    echo '<h3>Bug-Task Dependencies Export Format</h3> The Bug-Task
...
values indicate what the field data types are. <p>';
	...
	$sql_create = "CREATE TABLE $tbl_name (".
	    'bug_id INTEGER, group_id INTEGER, is_dependent_on_task_id INTEGER)';
