    ...
    if ($request->isPost() && $request->valid($vPostChanges)) {
        ...
        */
        ...
        if ($request->existAndNonEmpty('delete')) {
			...
			*/
            if (($current_project->getAccess() == Project::ACCESS_PRIVATE && ($is_public ==0 || $is_public ==9) ) || $current_project->getAccess() == Project::ACCESS_PUBLIC) {
		...
		*/
		...
		if (!$result || $rows < 1) {
			...
			echo '
                        ...
                        if ($current_project->getAccess() == Project::ACCESS_PUBLIC) {
                            echo '<P>
