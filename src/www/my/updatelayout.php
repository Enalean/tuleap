        ...
        case 'widget':
            if ($request->exist('name') && $request->exist('layout_id')) {
                ...
                if ($widget = Widget::getInstance($name)) {
                    switch(array_pop(array_keys($param[$name]))) {
                        case 'remove':
                            if ($widget->isUnique()) {
                                //delete
                                $sql = "DELETE FROM user_layouts_contents WHERE user_id = ". user_getid() ." AND layout_id = $layout_id AND name = '$name'";
                            } else {
                                //mark as deleted
                                $sql = "UPDATE user_layouts_contents SET column_id = 0 WHERE user_id = ". user_getid() ." AND layout_id = $layout_id AND name = '$name'";
                            }
                            db_query($sql);
                            echo db_error();
                            break;
