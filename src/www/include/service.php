...
function service_create_service($arr, $group_id, $template, $force_enable = false) {
    ...
    $sql    = "INSERT INTO service (group_id, label, description, short_name, link, is_active, is_used, scope, rank, server_id) VALUES ($group_id, '".$arr['label']."', '".$arr['description']."', '".$arr['short_name']."', '".$link."', ".$arr['is_active'].", ". ($force_enable ? 1 : $is_used) .", '".$arr['scope']."', ".$arr['rank'].", ". $server_id .")";
    $result = db_query($sql);
