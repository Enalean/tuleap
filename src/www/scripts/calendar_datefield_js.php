...
function show_calendar(str_target, str_datetime, css_theme_file, img_theme_path, datefields) {
    ...
    if (datefields != null) {
        document.datefields = datefields;
    }
	// If no date/time given then default to today at 00:00
	var re_date = /^(\d+)\-(\d+)\-(\d+)\s*$/;
	if (!re_date.exec(str_datetime) && !(str_datetime == null || str_datetime =="")) {
        var dt_datetime = new Date();
        is_date_field = 1;
    } else if (str_datetime == null || str_datetime =="") {
	...
	if ((dt_datetime.getMonth() + 1)%12 != dt_next_month.getMonth()%12)
      ...
      var str_buffer = new String (
                ...
                str_target_escaped+"', '"+dt2dtstr(dt_prev_year)+"','"+css_theme_file+"','"+img_theme_path+"', "+null+");\">"+
		...
		str_target_escaped+"', '"+ dt2dtstr(dt_prev_month)+"','"+css_theme_file+"','"+img_theme_path+"', "+null+");\">"+
                ...
                +str_target_escaped+"', '"+dt2dtstr(dt_next_month)+"','"+css_theme_file+"','"+img_theme_path+"', "+null+");\">"+
		...
		+str_target_escaped+"', '"+dt2dtstr(dt_next_year)+"','"+css_theme_file+"','"+img_theme_path+"', "+null+");\">"+
        ...
        // print calendar footer
          ...
          str_buffer +=
	         "</table>\n" +
                  "</tr>\n</td>\n</table>\n" +
                  "<p align='center'><b>Or Select a date field:</b></p>" +
                  "<p align='center'><select id='datefield' name='datefield' onChange=\"" +
                  "javascript:window.opener."+str_target+".value = window.opener.document.datefields[document.getElementById('datefield').value]; " + 
                  "window.opener."+str_target+"_fieldname.value = document.getElementById('datefield').value; " +
                  "window.close();\">"+
                  "<option value='0'>--None--</option>";
          for (key in document.datefields) {
              if (key != str_target.substring(23, str_target.length)) {
                  str_buffer += "<option value='"+key+"'>"+document.datefields[key]+"</option>";
              }
          }
          str_buffer += 
             "</select></p>" +
        ...
        var vWinCal = window.open("", "Calendar",
                "width=200,height=260,status=no,resizable=yes,top=200,left=200");
        vWinCal.opener = self;
        vWinCal.focus();
