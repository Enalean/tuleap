 ...
 * Copyright (c) Enalean SAS, 2011-2017. All rights reserved
 *
...
function load(toggler, force_display, force_ajax) {
    if (force_display) {
        const was_noajax = toggler.classList.contains('toggler-hide-noajax') ||
    ...
    if (force_ajax) {
        const was_hide = toggler.classList.contains('toggler-hide') ||
...
function listenClickOnToggle(evt) {
    const toggler = this,
          is_collapsing = toggler.classList.contains('toggler') || toggler.classList.contains('toggler-noajax');

    before(evt, toggler, is_collapsing);

    //toggle the state
    if (toggler.classList.contains('toggler-noajax') || toggler.classList.contains('toggler-hide-noajax')) {
