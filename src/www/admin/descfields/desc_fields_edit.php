...
$HTML->header(array('title'=>$Language->getText('admin_desc_fields', 'title')));
...
if($update_fields_desc_id){
	...
	echo (($row_update['desc_required']==1) ? " CHECKED" : "" )."></p>";
		...
		echo "<INPUT type='submit' name='Update' class='btn btn-primary tlp-button-primary' value='".$Language->getText('global','btn_update')."'>";
	echo "</form>";
}else{
	...
	echo "<HR>
		  <p><input type='submit' name='Add_desc' class='btn btn-primary tlp-button-primary' value=".$Language->getText('global','btn_submit')."></p>";
	echo "</form>";
}
$HTML->footer(array());
