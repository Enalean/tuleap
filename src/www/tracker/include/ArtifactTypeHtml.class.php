...
class ArtifactTypeHtml extends ArtifactType {
	...
	function adminHeader($params) {
	  global $Language;
		...
		echo ' | <a href="/tracker/admin/?group_id='.$group_id.'&atid='. $this->getID() .'&func=permissions">'.$Language->getText('tracker_include_type','permissions').'</a>';
        echo ' | <a href="/tracker/admin/?group_id='.$group_id.'&atid='. $this->getID() .'&func=fieldsets">'.$Language->getText('tracker_include_type','fieldsets').'</a>';
		...
		echo ' | <a href="/tracker/admin/?group_id='.$group_id.'&atid='. $this->getID() .'&func=field_dependencies">'.$Language->getText('tracker_include_type','field_dependencies').'</a>';
		...
		if (!$at_arr) {
			...
			while ($arr = db_fetch_array($at_arr)) {
			    echo sprintf($fmt,
					    ...
					    "<a href=\"/tracker/admin/restore.php?func=restore&group_id=".$arr['group_id']."&atid=".$arr['group_artifact_id']."\"><img src=\"".util_get_image_theme("ic/convert.png")."\" border=\"0\" onClick=\"return confirm('".$Language->getText('tracker_include_type','restore_tracker')."')\"></a>",
	...
	function displayAdminTracker($group_id,$atid) {
	  global $Language;
	    ...
	    if ( $this->userIsAdmin() ) {
            ...
            echo '<H3><a href="/tracker/admin/?group_id='.$group_id.'&atid='.$atid.'&func=permissions">'.$Language->getText('tracker_include_type','manage_permissions').'</a></H3>';
            echo $Language->getText('tracker_include_type','define_manage_permissions');
            echo '<H3><a href="/tracker/admin/?group_id='.$group_id.'&atid='.$atid.'&func=fieldsets">'.$Language->getText('tracker_include_type','manage_fieldsets').'</a></H3>';
			...
			echo '<H3><a href="/tracker/admin/?func=field_dependencies&group_id='.$group_id.'&atid='.$atid.'">'.$Language->getText('tracker_include_type','mng_field_dependencies').'</a></H3>';
        ...
        function displayPermissionsGeneralMenu() {
            ...
            $permissions = array(
                                 array(
                                       'link' => '/tracker/admin/?group_id='.$this->getGroupID().'&atid='.$this->getID().'&func=permissions&perm_type=tracker',
                                 ...
                                 array(
                                       'link' => '/tracker/admin/?group_id='.$this->getGroupID().'&atid='.$this->getID().'&func=permissions&perm_type=fields',
        ...
        function displayAdminTitle($title) {
            echo '<H2>',
                ...
                ' \'<a href="/tracker/admin/?group_id=',$this->getGroupID(),'&atid=',$this->getID(),'">',
                $this->getName(),
                '</a>\'',
                $title,
                '</H2>';
        ...
        function displayPermissionsFieldsTracker($ugroups_permissions, $group_first, $selected_id = false) {
            ...
            //form
            $url_action_without_group_first = "?group_id=".$this->getGroupID().'&atid='.$this->getID()."&func=permissions&perm_type=fields";
...
EOS;
            ...
            if (($group_first && count($ugroups_permissions) < 1) || (!$group_first && count($ugroups_permissions[$key]['related_parts']) < 1)) {
                ...
                $html .= "<div>";
                $html .= '<input type="hidden" name="selected_id" value="'.($selected_id?$selected_id:"false").'" />';
                //intro
                ...
                foreach($ugroups_permissions as $part_permissions) {
                    ...
                    $html .= "<option value='".$part_permissions['values']['id']."' ";
                ...
                foreach($related_parts as $ugroup_permissions) {
                    ...
                    $name = "<a href='".$url_action_without_group_first."&selected_id=".$second_part['id']."&group_first=".($group_first?0:1)."'>";
                    ...
                    //The permissions
                    {
                        ...
                        if ($group_first) {
                            $name_of_variable = "permissions[".$second_part['id']."][".$first_part['id']."]";
                        } else {
                            $name_of_variable = "permissions[".$first_part['id']."][".$second_part['id']."]";
                        }
                        $html .= "<input type='hidden' name='".$name_of_variable."[submit]' value='off'/>";//TODO verifier diff�rents navigateurs
                ...
                if ($a_star_is_displayed) {
                    $html .= $GLOBALS['Language']->getText('tracker_admin_permissions', 'ug_may_have_no_access',"/tracker/admin/?group_id=".$this->getGroupID()."&atid=".$this->getID()."&func=permissions&perm_type=tracker");
                }
            ...
            $html .= $GLOBALS['Language']->getText('project_admin_permissions',
                                                   ...
                                                   array(
                                                         "/project/admin/editugroup.php?func=create&group_id=".$this->getGroupID(),
                                                         "/project/admin/ugroup.php?group_id=".$this->getGroupID()
                                                         )
                                                   );
            $html .= "</p>";
            print $html;
        ...
        function displayPermissionsTracker($ugroups_permissions) {
            ...
            //form
            $html .= "<form name='form_tracker_permissions' action='?group_id=".$this->getGroupID().'&atid='.$this->getID()."&func=permissions&perm_type=tracker' method='post'>";
            $html .= "<div>";
			     ...
			     <td>
			     	<table>
			     	  <tr>
			     		...
			     		<td><input name="group_id_template" value="'.$group_id_template.'"><a href="javascript:showGroupSelection()"><img src="'.util_get_image_theme("button_choose.png").'" align="absmiddle" border="0"></a></td>
			     	  </tr>
			     	  <tr>
				        <td>'.$Language->getText('tracker_include_type','tracker_id').'</td>
			     		<td><input name="atid_template" value="'.$atid_template.'"><a href="javascript:showTrackerSelection()"><img src="'.util_get_image_theme("button_choose.png").'" align="absmiddle" border="0"></a></td>
					  <tr>
					</table>
				 </td>			     		
	...
	function displayFieldUsageList() {
		global $ath,$art_field_fact,$art_fieldset_fact,$PHP_SELF,$Language;
        ...
        $html = "";
		$tracker_url = $PHP_SELF.'?group_id='.$this->Group->getID().'&atid='.$this->getID();
        ...
        while (list($fieldset_id, $fieldset) = each($fieldsets_with_used_fields)) {
            ...
            $html .= '<tr class="fieldset_separator"><td colspan="7">'.$fieldset->getLabel().'</td></tr>';
            while (list($key, $field) = each($used_fields_in_fieldset) ) {
                ...
                $html .= '<TD><A HREF="'.$tracker_url.'&func=display_field_update&field_id='.$field->getID().'">'.
                $field->getLabel().'</A></td>'.
                "\n<td>".$field->getLabelFieldType().'</td>'.
                "\n<td>".$field->getDescription().'</td>'.
                "\n<td><a href=\"".$tracker_url."&func=display_fieldset_update&fieldset_id=".$fieldset->getID()."\">".$fieldset->getLabel().'</td>'.
			...
			if ( $use_it == 1 ) {
			  ...
			  if ( $field && $field->isSpecial() && ($field->getName() != "comment_type_id") && !user_is_super_user()) {
			    ...
			    echo '<input type="checkbox" name="use_it" value="1">';
                        ...
                        echo '
                      </td><td><a href="/tracker/admin/?group_id='.$this->Group->getID().'&atid='.$this->getID().'&func=permissions&perm_type=fields&selected_id='.$field_id.'&group_first=0">'.$Language->getText('tracker_include_type','edit_field_perm').'</a>';
	...
	function displayFieldValuesEditList() {
		...
		while (list($fieldset_id, $fieldset) = each($fieldsets)) {
            ...
            $html .= '<tr class="fieldset_separator"><td colspan="3">'.$fieldset->getLabel().'</td></tr>';
            ...
            while (list($field_name,$field) = each($fields)) {
              ...
              if ( (($field->getName() != "comment_type_id")&&($field->isSpecial())) ||
                ...
                $html .= '<TD><A HREF="?group_id='.$this->Group->getID()."&atid=".$this->getID().
                '&func=display_field_values&field_id='.$field->getID().'">'.
                $field->getLabel().'</A></td>'.
                "\n<td>".$field->getDescription().'</td>';
                
                $html .= '<td>'.$fieldset->getLabel().'</td>';
	...
	function displayValueFunctionForm($field_id,$value_function,$or_label="") {
	  global $Language;
	    ...
	    echo '<option value="group_members"'.$selected.'>'.$name.'</option>';
	    ...
	    echo '<option value="group_admins"'.$selected.'>'.$name.'</option>';
	    ...
	    echo '<option value="tracker_admins"'.$selected.'>'.$name.'</option>';
		...
		$selected = " selected";
	      ...
	      echo '<option value="'.$ugr.'"'.$selected.'>'.util_translate_name_ugroup(db_result($ugroup_res, $i, 'name')).'</option>';	      
	    }
	...
	*/
    ...
    function displayFieldSetCreateForm($func="fieldset_create",$fieldset_id=false,$fieldset_name=false,$description=false,$rank=false) {
        ...
        if ( $func == "fieldset_create" ) {
            ...
            echo '
              ...
              <input type="hidden" name="fieldset_name" value="'.$fieldset_name.'">
        ...
        echo '<input type="text" name="name" id="name" value="'.($fieldset_name?$fieldset_name:"").'" size="30" maxlength="40" />';
        echo '</p>';
        echo '<p>';
        echo '<label for="description">'.$Language->getText('tracker_include_type','fieldset_desc').':</label>';
        echo '<input type="text" name="description" id="description" value="'.($description?$description:"").'" size="70" maxlength="255" />';
        echo '</p>';
        echo '</fieldset>';
        echo '<fieldset>';
        ...
        echo '<input type="text" name="rank" id="rank" value="'.($rank?$rank:"").'" size="5" maxlength="5" />';
        echo '</p>';
        echo '</fieldset>';
    ...
    function displayFieldSetList() {
        global $ath,$art_fieldset_fact,$PHP_SELF,$Language;
        ...
        foreach ($fieldsets as $fieldset) {
            ...
            $html .= '<tr class="'.util_get_alt_row_color($iu) .'">';
            $html .= '<td><a href="'.$PHP_SELF.'?group_id='.$this->Group->getID()."&atid=".$this->getID().'&func=display_fieldset_update&fieldset_id='.$fieldset->getID().'">'.$fieldset->getLabel().'</A></td>';
            $html .= '<td>'.$fieldset->getDescriptionText().'</td>';
            $html .= '<td>';
            $fields_contained = $fieldset->getArtifactFields();
            if (count($fields_contained) > 0) {
                $html .= '<ul>';
                foreach($fields_contained as $field_contained) {
                    $link_field_usage = '/tracker/admin/?func=display_field_update&group_id='.$this->Group->getID().'&atid='.$this->getID().'&field_id='.$field_contained->getID();
                    $html .= '<li>';
                    if ($field_contained->getUseIt()) {
                        $html .= '<strong><a href="'.$link_field_usage.'">'.$field_contained->getLabel().'</a></strong>';
                    } else {
                        $html .= '<em><a href="'.$link_field_usage.'">'.$field_contained->getLabel().'</a></em>';
                    }
                    $html .= '</li>';
                }
                $html .= '</ul>';
            ...
            if (count($fields_contained) <= 0) {
                $html .= '<a href="/tracker/admin/?func=fieldset_delete&group_id='.$this->Group->getID().'&atid='.$this->getID().'&fieldset_id='.$fieldset->getID().'"><img src="'.util_get_image_theme("ic/trash.png").'" border="0" onClick="return confirm(\''.$Language->getText('tracker_include_type','warning_delete_fieldset').'\')"></a>';
            }
            $html .= '</td>';
            $html .= '</tr>';
