                    ...
                    if (!$res_file || $num_files < 1) {
                        ...
                        foreach($res_file as $file_release) {
                            ...
                            if (($package->getApproveLicense() == 0) && (isset ($GLOBALS['sys_frs_license_mandatory']) && !$GLOBALS['sys_frs_license_mandatory'])) {
                                ...
                                // Display popup
                                print '<A HREF="javascript:showConfirmDownload(' . $group_id . ',' . $file_release['file_id'] . ',\'' . $hp->purify($file_release['filename'], CODENDI_PURIFIER_JS_QUOTE) . '\')" title="' . $file_release['file_id'] . " - " . $hp->purify($fname) . '">' . $hp->purify($fname) . '</A>';
                            }
                            $size_precision = 0;
