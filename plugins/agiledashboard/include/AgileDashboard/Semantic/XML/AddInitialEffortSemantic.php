...
final class AddInitialEffortSemantic
{
    public function process(\SimpleXMLElement $xml_tracker, PlatformConfiguration $platform_configuration, FieldMappingCollection $field_mapping_collection): void
    {
        ...
        $story_points_field = $field_mapping_collection->getMappingFromJiraField($platform_configuration->getStoryPointsField());
        if (! $story_points_field) {
            return;
        }

        $semantic_node = $xml_tracker->semantics->addChild('semantic');
        ...
        $semantic_node->addChild("description", dgettext('tuleap-agiledashboard', 'Define the initial effort of an artifact.'));
