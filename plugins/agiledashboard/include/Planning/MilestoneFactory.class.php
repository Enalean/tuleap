...
class Planning_MilestoneFactory {
    ...
    public function getMilestoneWithAssignedArtifacts($user, $group_id, $planning_id, $artifact_id) {
        $planning = $this->planning_factory->getPlanningWithTrackers($planning_id);
        $artifact = $this->artifact_factory->getArtifactById($artifact_id);
        
        if ($artifact) {
            $content_tree = $this->getAssignedArtifacts($user, $planning, $artifact, 1);
    ...
    private function getAssignedArtifacts($user, $planning, $milestone_artifact, $child_depth) {
        ...
        $node->setId($id);
        $this->addChildrenAssignedArtifacts($user, $milestone_artifact, $node, $child_depth);
        
        return $node;
    }
    
    private function addChildrenAssignedArtifacts($user, $artifact, $parent_node, $child_depth = 0) {
        ...
        foreach ($linked_artifacts as $linked_artifact) {
            ...
            if ($child_depth > 0) {
                $this->addChildrenAssignedArtifacts($user, $linked_artifact, $node, $child_depth - 1);
            }
