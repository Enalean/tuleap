describe('InPropertiesItemFilter', function() {
    ...
    var in_properties_filter,
        list = [
            {label: 'Riri', id: 'nephew', card_fields: []},
            {label: 'Fifi', id: 'nephew', card_fields: []},
            {label: 'Loulou', id: 'nephew', card_fields: []},
            {label: 'Donald', id: 'uncle', card_fields: [], internal_property: 'has nephews'},
            {label: 'Donald & Daisy', id: 'significant others', card_fields: []}
        ];
    ...
    it('returns items that have matching card_fields', function () {
        ...
        expect(in_properties_filter([{id: null, label: null, card_fields: [
            { type: 'computed', value: 42}
        ]}], '42').length).toBe(1);
        expect(in_properties_filter([{id: null, label: null, card_fields: [
            { type: 'priority', value: 42}
        ]}], '42').length).toBe(1);
        expect(in_properties_filter([{id: null, label: null, card_fields: [
            { type: 'file', file_descriptions: [{ name: 'Photo de Toto.png'}]}
        ]}], 'toto').length).toBe(1);
        expect(in_properties_filter([{id: null, label: null, card_fields: [
            { type: 'cross', value: [ { ref: 'release #42' } ]}
        ]}], '42').length).toBe(1);
        expect(in_properties_filter([{id: null, label: null, card_fields: [
        ...
        ['sb', 'rb', 'cb', 'msb', 'tbl', 'shared'].forEach(function (list_type) {
          ...
          expect(in_properties_filter([{id: null, label: null, card_fields: [
              { type: list_type, values: [{ display_name: 'Mr Toto'}]}
          ]}], 'toto').length).toBe(1);
        });
        ['date', 'lud', 'subon'].forEach(function (date_type) {
          ...
          expect(in_properties_filter([{id: null, label: null, card_fields: [
              { type: date_type, value: today.toJSON() }
          ]}], 'today').length).toBe(1);
        });
    });
});
