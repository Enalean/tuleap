...
class AgileDashboard_Milestone_MilestoneStatusCounterTest extends TuleapTestCase {
    ...
    public function setUp() {
        ...
        $this->counter = new AgileDashboard_Milestone_MilestoneStatusCounter($this->backlog_dao, $this->artifact_dao);
    }

    public function itDoesntFetchAnythingWhenNoMilestoneId() {
        expect($this->backlog_dao)->getBacklogArtifacts()->never();
        $result = $this->counter->getStatus(null);
    ...
    public function itReturnsZeroOpenClosedWhenNoArtifacts() {
        ...
        $result = $this->counter->getStatus(12);
        $this->assertEqual($result, array(
           Tracker_ArtifactDao::STATUS_OPEN   => 0,
           Tracker_ArtifactDao::STATUS_CLOSED => 0,
        ));
