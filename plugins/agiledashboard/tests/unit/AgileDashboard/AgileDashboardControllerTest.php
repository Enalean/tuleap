...
final class AgileDashboardControllerTest extends \Tuleap\Test\PHPUnit\TestCase //phpcs:ignore PSR1.Classes.ClassDeclaration.MissingNamespace
{
    ...
    public function testItDoesNotCreateAKanbanIfTrackerIsUsedInScrumBacklog(): void
    {
        $tracker = Mockery::spy(\Tracker::class);

        $this->planning_factory->shouldReceive('getPlanningsByBacklogTracker')->with($tracker)->andReturns([]);

        $controller = $this->getMockedController($tracker);

        $this->kanban_manager->shouldReceive('createKanban')->never();

        $controller->createKanban();
    }

    public function testItDoesNotCreateAKanbanIfTrackerIsUsedInScrumPlanning(): void
    {
        $tracker = Mockery::spy(\Tracker::class);

        $this->planning_factory->shouldReceive('getPlanningByPlanningTracker')->with($tracker)->andReturns([]);
    ...
    private function getMockedController(Tracker $tracker): AdminController
    {
        ...
        $this->request->shouldReceive('get')->with('kanban-name')->andReturns('My Kanban');
        $this->request->shouldReceive('get')->with('group_id')->andReturns(789);
        $this->request->shouldReceive('get')->with('tracker-kanban')->andReturns(123);
        $this->request->shouldReceive('getCurrentUser')->andReturns($admin_user);
        $this->request->shouldReceive('getProject')->andReturns(Mockery::mock(Project::class));
        $this->tracker_factory->shouldReceive('getTrackerById')->with(123)->andReturns($tracker);
        $this->kanban_manager->shouldReceive('doesKanbanExistForTracker')->with($tracker)->andReturns(true);
        $this->user_manager->shouldReceive('getCurrentUser')->andReturns($admin_user);
