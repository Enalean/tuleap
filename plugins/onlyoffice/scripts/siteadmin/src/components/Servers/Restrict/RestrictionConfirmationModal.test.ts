...
describe("RestrictionConfirmationModal", () => {
    it("should cancel restriction if user closes the modal", async () => {
        const wrapper = shallowMount(RestrictionConfirmationModal, {
            global: { plugins: [createGettext({ silent: true })] },
        });

        expect(wrapper.emitted("cancel-restriction")).toBeFalsy();

        await wrapper.find("[data-dismiss=modal]").trigger("click");

        expect(wrapper.emitted("cancel-restriction")).toBeTruthy();
    });

    it("should inform user that something is happening when user confirm the restriction", async () => {
        const wrapper = shallowMount(RestrictionConfirmationModal, {
            global: { plugins: [createGettext({ silent: true })] },
        });
