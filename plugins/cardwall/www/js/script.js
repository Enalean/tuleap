...
document.observe('dom:loaded', function () {
    ...
    (function displayBurndownChartWithD3() {
        var burndown = {
            duration: 10,
            capacity: 50,
            points: [ 50, 75, 42, 35, 29, 24, 18, 4 ]
        }
        ...
        var svg,
            ...
            getXCoordinateForAGivenBurndownPoint = function(d, index) {
                return x(index) / burndown.duration;
            },
            ideal_data  = [ burndown.capacity, 0 ],
            width       = 300,
            height      = 85;

        (function defineAbscissas() {
            x = d3.scale.linear().range([0, width]);
            x.domain(d3.extent(ideal_data, function(d, index){return index;}));
        })();

        (function defineOrdinates() {
            y = d3.scale.linear().range([height, 0]);
            y.domain(d3.extent(burndown.points.concat(ideal_data)));
        })();

        (function defineLineFunctions() {
            idealLine = d3.svg.line()
                .x(function(d, index) { return x(index); })
                .y(y);
            actualLine = d3.svg.line()
                .x(getXCoordinateForAGivenBurndownPoint)
                .y(y);
        })();

        (function bootstrapTheChart() {
            svg = d3.select(".milestone-burndown").append("svg")
                .attr("class", "chart")
                .attr("width", width)
                .attr("height", height);
        ...
        (function paintTheActualLine() {
            chart.append("path")
                .datum(burndown.points)
                .attr("class", "line actual")
                .attr("d", actualLine);
        })();

        (function paintTheDots() {
            chart.selectAll('circle')
                ...
                .enter()
                    ...
                    .attr("r", "4")
                    .attr("cx", getXCoordinateForAGivenBurndownPoint)
                    .attr("cy", y)
                    .append("title")
