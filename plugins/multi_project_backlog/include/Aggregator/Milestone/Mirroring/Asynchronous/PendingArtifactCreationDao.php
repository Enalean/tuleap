...
class PendingArtifactCreationDao extends DataAccessObject
{
    public function getPendingArtifactById(int $artifact_id, int $user_id): ?array
    {
        return $this->getDB()->row(
            'SELECT aggregator_artifact_id, user_id, changeset_id FROM plugin_multi_project_backlog_pending_mirrors WHERE aggregator_artifact_id = ? AND user_id = ?',
            $artifact_id,
            $user_id
        );
    }
    ...
    {
        $this->getDB()->run(
            "INSERT INTO plugin_multi_project_backlog_pending_mirrors
            (aggregator_artifact_id, user_id, changeset_id) VALUES (?, ?, ?)",
            $artifact_id,
            $user_id,
            $changeset_id
        );
    ...
    {
        $this->getDB()->run(
            "DELETE FROM plugin_multi_project_backlog_pending_mirrors
            WHERE aggregator_artifact_id = ? AND user_id = ?",
            $artifact_id,
            $user_id
        );
    }
