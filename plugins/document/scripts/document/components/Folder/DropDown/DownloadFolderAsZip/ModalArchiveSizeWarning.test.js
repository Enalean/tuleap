...
describe("ModalArchiveSizeWarningModal", () => {
    ...
    it("When user confirms, it downloads the zip and closes the modal", () => {
        const wrapper = getWrapper();
        ...
        wrapper
            .find("[data-test=confirm-download-archive-button-despite-size-warning]")
            .trigger("click");

        expect(wrapper.emitted("download-folder-as-zip-modal-closed").length).toBe(1);
        expect(window.location.assign).toHaveBeenCalledWith("/download/me/here");
    });
});
