...
describe("UnlockItem", () => {
    ...
    async function createWrapper(item: ItemFile): Promise<Wrapper<UnlockItem>> {
    ...
    it(`Given document is not locked
        ...
        Then I should not be able to unlock it`, async () => {
        const item = {
            id: 1,
            user_can_write: true,
            lock_info: null,
    ...
    it(`Given user can write
        When we display the dropdown
        Then I should able to unlock any item locked`, async () => {
        const item = {
            id: 1,
            user_can_write: true,
            lock_info: {
    ...
    it(`Given item is a file and given user can write
        Then unlock option should be displayed`, async () => {
        const item = {
            id: 1,
            user_can_write: true,
            lock_info: {
    ...
    it(`unlock document on click`, async () => {
        const item = {
            id: 1,
            user_can_write: true,
            lock_info: {
