...
function ReleaseRestService(
    ...
    function getReleaseLinkNatures(artifact_id) {
        return $http.get('/api/v1/artifacts/' + artifact_id + '/links', {
            cache  : true,
            timeout: 10000
        })
    ...
    function recursiveGetLinkedArtifacts(uri, limit, offset, progress_callback) {
        return self.getLinkedArtifacts(uri, limit, offset)
        .then(function(response) {
            ...
            return recursiveGetLinkedArtifacts(
                uri,
                limit,
                offset + limit,
                progress_callback
