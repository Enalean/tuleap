...
class TestPlanTestDefinitionsTestStatusDAO extends DataAccessObject
{
    ...
    public function searchTestStatusPerTestDefinitionInAMilestone(TestPlanMilestoneInformationNeededToRetrieveTestStatusPerTestDefinition $information): array
    {
        ...
        JOIN (
            SELECT test_def_id, test_exec_changeset_id, test_campaign_id
            # We need to find the most recent test_exec_changeset but since we also need to retrieve
            # the corresponding test definition ID *and* the campaign ID we cannot rely on MAX(test_exec_changeset_id):
            # we need to solve a "top-1 in each group" type of problem.
            # MySQL 5.7 does not have windowing functions so we need to workaround it.
            # The query rely upon SQL variables to find the first row of each group and make sure the first row of each
            # group is the most recent changeset with an ORDER BY clause. This approach has a complexity of O(N) where N
            # is the number of rows to filter.
            # It also can be solved with an uncorrelated sub-query with a complexity of O(M) where M is the number of test
            # definitions (if MySQL successfully auto-generates an index for the sub-query) but that implies to duplicate
            # the sub-query dealing the retrieval of the most recent test exec changeset per test def in the campaigns
            # which would hurt the maintainability and readability of the whole query.
            FROM (SELECT @previous := 0) AS initialization
            JOIN (
                SELECT test_def_id != @previous AS first, @previous := test_def_id, test_def_id, test_exec_changeset_id, test_campaign_id
                FROM (
                    ...
                    SELECT test_def.id AS test_def_id, test_exec_changeset.id AS test_exec_changeset_id, test_campaign_id
                ...
                ORDER BY test_def_id, test_exec_changeset_id DESC, test_campaign_id DESC
            ) AS non_filtered_data
            WHERE first = 1
