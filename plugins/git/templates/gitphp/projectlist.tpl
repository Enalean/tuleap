...
{t}Search projects{/t}: <input type="text" name="s" class="projectSearchBox" {if $search}value="{$search|escape}"{/if} /> <a href="index.php" class="clearSearch" {if !$search}style="display: none;"{/if}>X</a> {if $javascript}<img src="images/search-loader.gif" class="searchSpinner" style="display: none;" />{/if}
</form>
</div>
  ...
  {foreach name=projects from=$projectlist item=proj}
    ...
    <tr class="{cycle values="light,dark"} projectRow">
      <td class="projectName">
        <a href="{$SCRIPT_NAME}?p={$proj->GetProject()|urlencode}&amp;a=summary" class="list {if $currentcategory != ''}indent{/if}">{$proj->GetProject()|escape}</a>
      </td>
      {assign var=projecthead value=$proj->GetHeadCommit()}
      <td class="projectAge">
        {if $projecthead}
          {if $proj->GetAge() < 7200}   {* 60*60*2, or 2 hours *}
            <span class="agehighlight"><strong><em>{$proj->GetAge()|agestring|escape}</em></strong></span>
          {elseif $proj->GetAge() < 172800}   {* 60*60*24*2, or 2 days *}
            <span class="agehighlight"><em>{$proj->GetAge()|agestring|escape}</em></span>
          {else}
            <em>{$proj->GetAge()|agestring|escape}</em>
          {/if}
	{else}
	...
	<a href="{$SCRIPT_NAME}?p={$proj->GetProject()|urlencode}&amp;a=snapshot&amp;h=HEAD&amp;noheader=1" class="snapshotTip">{t}snapshot{/t}</a>
	{/if}
      </td>
    </tr>
  {foreachelse}
