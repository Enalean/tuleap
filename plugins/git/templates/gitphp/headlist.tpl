 ...
 <table cellspacing="0">
   ...
   {foreach from=$headlist item=head name=heads}
       ...
       <tr class="{cycle values="light,dark"}">
         ...
         <td class="link"><a href="{$SCRIPT_NAME}?a=shortlog&amp;h=refs/heads/{$head->GetName()|urlencode}">{t}shortlog{/t}</a> | <a href="{$SCRIPT_NAME}?a=log&amp;h=refs/heads/{$head->GetName()|urlencode}">{t}log{/t}</a> | <a href="{$SCRIPT_NAME}?a=tree&amp;hb={$headcommit->GetHash()}">{t}tree{/t}</a></td>
       </tr>
   {/foreach}
   {if $hasmoreheads}
       <tr>
