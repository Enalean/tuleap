...
class GitMarkdownFile {
     ...
     * @return array
     */
    public function getReadmeFileContent($path, $node, $commit_sha1) {
        $readme_file = $this->getReadmeFile($path, $node, $commit_sha1);
        if ($readme_file && version_compare(phpversion(), '5.3', '>=')){
            return $this->getFormatedMarkdown($readme_file, $path, $commit_sha1);
        }

        return false;
    }

    private function getFormatedMarkdown($file_name, $path, $commit_sha1) {
    ...
    private function getReadmeFile($path, $node, $commit_sha1) {
        $files_list          = $this->git_exec->lsTree($commit_sha1, $node);
        $markdown_files_list = array_values(preg_grep('/^readme\.(markdown|mdown|mkdn|md|mkd|mdwn|mdtxt|mdtext|text)$/i', $files_list));
