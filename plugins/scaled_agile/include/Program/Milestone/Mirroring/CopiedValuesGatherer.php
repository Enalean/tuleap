...
class CopiedValuesGatherer
{
    ...
    public function gather(
        \Tracker_Artifact_Changeset $program_milestone_last_changeset,
        \Tracker $program_top_milestone_tracker
    ): CopiedValues {
        $fields            = $this->fields_gatherer->gather($program_top_milestone_tracker);
        $title_value       = $this->readTitle($fields, $program_milestone_last_changeset);
        $description_value = $this->readDesription($fields, $program_milestone_last_changeset);
        $status_value      = $this->readStatus($fields, $program_milestone_last_changeset);
        $start_date_value  = $this->readStartDate($fields, $program_milestone_last_changeset);
        $end_period_value  = $this->readEndPeriod($fields, $program_milestone_last_changeset);

        return new CopiedValues(
            $title_value,
            $description_value,
            $status_value,
            (int) $program_milestone_last_changeset->getSubmittedOn(),
            (int) $program_milestone_last_changeset->getArtifact()->getId(),
            $start_date_value,
            $end_period_value
        );
    }
    ...
    private function readTitle(
        ...
        $title_value = $program_milestone_last_changeset->getValue($title_field);
        if (! $title_value) {
            throw new NoTitleChangesetValueException(
                (int) $program_milestone_last_changeset->getId(),
    ...
    private function readDesription(
        ...
        $description_value = $program_milestone_last_changeset->getValue($description_field);
    ...
    private function readStatus(
        ...
        $status_value = $program_milestone_last_changeset->getValue($status_field);
        if (! $status_value) {
    ...
    private function readStartDate(
        ...
        $start_date_value = $program_milestone_last_changeset->getValue($start_date_field);
    ...
    private function readEndPeriod(
        ...
        $end_period_value = $program_milestone_last_changeset->getValue($end_period_field);
