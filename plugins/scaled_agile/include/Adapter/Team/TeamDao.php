...
final class TeamDao extends DataAccessObject implements TeamStore
{
    ...
    public function save(TeamCollection $team_collection): void
    {
        $this->getDB()->tryFlatTransaction(function () use ($team_collection): void {
            $sql = 'DELETE FROM plugin_scaled_agile_team_projects WHERE program_project_id = ?';
    ...
    public function getProgramIncrementOfTeam(int $team_project_id): ?int
    {
        ...
        $program_increment = $this->getDB()->single($sql, [$team_project_id]);

        if ($program_increment  === false) {
            return null;
        }
