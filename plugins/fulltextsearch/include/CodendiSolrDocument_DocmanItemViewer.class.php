...
class CodendiSolrDocument_DocmanItemViewer implements ICodendiSolrDocumentViewer {
    ...
    public function getHTMLResult() {
        ...
        if ($this->solr_doc->document_url) {
            ...
            $doc_url = '/plugins/docman/?group_id='.$this->solr_doc->forge_project_id.'&action=details&id='.$this->solr_doc->doc_item_id;
        }
        $doc_date = $this->solr_doc->doc_create_date;
        $doc_date = str_replace("T", " ", $doc_date);
        $doc_date = str_replace("Z", " ", $doc_date);
        $owner_name = $user_helper->getLinkOnUserFromUserId($this->solr_doc->doc_owner);

        $html .= '<p class="fulltextsearch_result_title">';
        $html .= '<a href="'.$doc_url.'" title="doc #'.$this->solr_doc->doc_item_id.'">'.$this->solr_doc->doc_title.'</a>';
        $html .= '</p>';
        $html .= '<p class="fulltextsearch_result_description">';
        $html .= $this->solr_doc->doc_description;
        $html .= '</p>';
