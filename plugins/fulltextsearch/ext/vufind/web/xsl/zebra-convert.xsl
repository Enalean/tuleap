...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="doc">
    ...
    <record>
      <id><xsl:value-of select=".//marc:datafield[@tag='999']/marc:subfield[@code='c']"/></id>

      <xsl:if test="./arr[@name='format']">
        <format><xsl:value-of select="./arr[@name='format']/str"/></format>
      </xsl:if>
      ...
      <xsl:for-each select=".//marc:datafield[@tag='856']">
        <url><xsl:value-of select=".//marc:datafield[@tag='100']/marc:subfield[@code='u']"/></url>
      </xsl:for-each>
