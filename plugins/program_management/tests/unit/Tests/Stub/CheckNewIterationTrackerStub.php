...
final class CheckNewIterationTrackerStub implements CheckNewIterationTracker
{
    ...
    public function checkIterationTrackerIsValid(
        ...
        if ($this->is_not_found) {
            throw new PlanTrackerNotFoundException($iteration_tracker_id);
        }
        if ($this->is_not_in_program) {
            throw new PlanTrackerDoesNotBelongToProjectException($iteration_tracker_id, $program->id);
        }
    }
}
