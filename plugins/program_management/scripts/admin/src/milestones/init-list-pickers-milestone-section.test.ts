...
import * as iterationLabelsInputHelper from "../helper/iteration-labels-input-helper";

const createDocument = (): Document => document.implementation.createHTMLDocument();

jest.mock("../helper/disabled-plannable-tracker-helper");
jest.mock("../helper/disabled-iteration-tracker-helper");
jest.mock("../helper/iteration-labels-input-helper");
describe("initListPickersMilestoneSection", () => {
    ...
    it("When all sectors exist, Then listpicker is called 4 times", async () => {
        ...
        const disabled_iteration_input = jest.spyOn(
            iterationLabelsInputHelper,
        ...
        expect(disabled_iteration_input).not.toHaveBeenCalledWith(doc);
    });

    it("When iteration tracker selector selects the first empty element, Then the input labels are disabled", async () => {
        const pi_selector = document.createElement("select");
        pi_selector.id = "admin-configuration-program-increment-tracker";
        ...
        const iteration_selector = document.createElement("select");
        iteration_selector.id = "admin-configuration-iteration-tracker";
        iteration_selector.options.add(new Option("", "", false, true));
        iteration_selector.options.add(new Option("Feature", "895", false, false));
        ...
        expect(disabled_iteration_input).toHaveBeenCalledWith(doc);
    });
});
