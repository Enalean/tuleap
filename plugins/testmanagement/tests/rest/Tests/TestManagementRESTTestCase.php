...
abstract class TestManagementRESTTestCase extends RestBase
{
    ...
    protected int $project_with_attachment_id;
    protected ?array $closed_71_campaign = null;
    protected ?array $valid_73_campaign  = null;
    protected ?array $valid_130_campaign = null;

    #[\Override]
    protected function getResponse(RequestInterface $request, string $user_name = TestManagementDataBuilder::USER_TESTER_NAME): ResponseInterface
    {
    ...
    public function setUp(): void
    {
        ...
        $this->valid_73_campaign = $ttm_cache->getValidCampaign();
        if ($this->valid_73_campaign === null) {
            $this->valid_73_campaign = $this->getValid73Campaign();
            $ttm_cache->setValidCampaign($this->valid_73_campaign);
        }

        $this->closed_71_campaign = $ttm_cache->getClosedCampaign();
        if ($this->closed_71_campaign === null) {
            $this->closed_71_campaign = $this->getClosed71Campaign();
            $ttm_cache->setClosedCampaign($this->closed_71_campaign);
        }

        $this->valid_130_campaign = $ttm_cache->getValidWithAttachmentsCampaign();
        if ($this->valid_130_campaign === null) {
            $this->valid_130_campaign = $this->getValid130Campaign();
            $ttm_cache->setValidWithAttachmentsCampaign($this->valid_130_campaign);
        }
    }

    private function getValid73Campaign(): array
    {
        ...
        self::assertSame('Tuleap 7.3', $campaign['label']);
        self::assertTrue($campaign['user_can_close']);
        self::assertTrue($campaign['user_can_open']);

        return $campaign;
    }

    private function getValid130Campaign(): array
    {
        ...
        self::assertSame('Tuleap 13.0', $campaign['label']);

        return $campaign;
    }

    private function getClosed71Campaign(): array
    {
        $all_campaigns_request  = $this->request_factory->createRequest('GET', "projects/$this->project_id/testmanagement_campaigns?" .
        ...
        self::assertSame('Tuleap 7.1', $campaign['label']);
