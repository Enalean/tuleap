 ...
 */
function _nodeListToTreeR($listOfNodes, $nodeId) {
    $children = null;
    if (array_key_exists($nodeId, $listOfNodes)) {
        foreach ($listOfNodes[$nodeId] as $child) {
            $children[$child] = _nodeListToTreeR($listOfNodes, $child);
        }
    }
 ...
 */
function _findRoot($listOfNodes) {
 ...
 */
function nodeListToTree($listOfNodes) {
    $root = _findRoot($listOfNodes);
    return array($root => _nodeListToTreeR($listOfNodes, $root));
}

/**
 * Megre two trees and tag the nodes with the information: IN_FIRST, IN_SECOND, IN_BOTH (recursive)
 */
function _array_merge_tag_recursive($array1, $array2) {
    $res = null;
    ...
    if ($array1 != null && isset($array1['children'])) {
        foreach ($array1['children'] as $name1 => $node1) {
            if (isset($array2['children']) && array_key_exists($name1, $array2['children'])) {
                $res['children'][$name1] = _array_merge_tag_recursive($array1['children'][$name1], $array2['children'][$name1]);
                $res['children'][$name1]['tag'] = 'IN_BOTH';
            } else {
                $res['children'][$name1] = $node1;
                _tagTree($res['children'][$name1], 'IN_FIRST');
            }
        }
    }
    ...
    if ($array2 != null && isset($array2['children'])) {
        foreach ($array2['children'] as $name2 => $node2) {
            if (!isset($res['children'][$name2])) {
                ...
                _tagTree($res['children'][$name2], 'IN_SECOND');
            }
        }
    }
 ...
 */
function _tagTree(&$tree, $tag) {
    $tree['tag'] = $tag;
    if (isset($tree['children'])) {
        foreach ($tree['children'] as $name => $node) {
            _tagTree($tree['children'][$name], $tag);
        }
    }
}
 ...
 */
function array_merge_tag_recursive($array1, $array2) {
    $root1 = array_pop(array_keys($array1));
    $root2 = array_pop(array_keys($array2));

    $res['(root)'] =  _array_merge_tag_recursive($array1[$root1], $array2[$root2]);
