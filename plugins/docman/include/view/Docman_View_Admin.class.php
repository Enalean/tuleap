...
class Docman_View_Admin extends Docman_View_Extra
{
    ...
    public function _content($params)
    {
        ...
        $html .= '<h3><a href="' . DocmanViewURLBuilder::buildUrl($params['default_url'], ['action' => 'admin_view']) . '">' . dgettext('tuleap-docman', 'Manage Display Preferences') . '</a></h3>';
        $html .= '<p>' . dgettext('tuleap-docman', 'Define the default view for the document manager.') . '</p>';

        $html .= '<h3><a href="' . DocmanViewURLBuilder::buildUrl($params['default_url'], ['action' => 'admin_metadata']) . '">' . dgettext('tuleap-docman', 'Manage Properties') . '</a></h3>';
        $html .= '<p>' . dgettext('tuleap-docman', 'Define the properties of your documents.') . '</p>';

        $html .= '<h3><a href="' . DocmanViewURLBuilder::buildUrl($params['default_url'], ['action' => 'report_settings']) . '">' . dgettext('tuleap-docman', 'Manage Search Report') . '</a></h3>';
        $html .= '<p>' . dgettext('tuleap-docman', 'Modify the scope of the reports.') . '</p>';

        $html .= '<h3><a href="' . DocmanViewURLBuilder::buildUrl($params['default_url'], ['action' => 'admin_obsolete']) . '">' . dgettext('tuleap-docman', 'Manage Obsolete Documents') . '</a></h3>';
        $html .= '<p>' . dgettext('tuleap-docman', 'View and update obsolete documents.') . '</p>';

        $html .= '<h3><a href="' . DocmanViewURLBuilder::buildUrl($params['default_url'], ['action' => 'admin_lock_infos']) . '">' . dgettext('tuleap-docman', 'Locked Documents') . '</a></h3>';
