...
class PHPWikiCloner {
 ...
 /**
   ...
   *
   */
  function PHPWikiCloner($template_id=0, $group_id=0) {
 ...
 /**
  ...
  function newWikiIsUsed(){
      if($this->new_wiki_is_used === null) {
         $res = db_query(sprintf("SELECT is_used FROM service WHERE group_id=%d AND short_name='%s'", $this->group_id, $this->escapeString("plugin_phpwiki")));
 ...
 /**
  ...
  function templateWikiHaveAttachments() { 
    $res = db_query('SELECT count(*) AS nb FROM plugin_phpwiki_attachment'
                    .' WHERE group_id='.db_ei($this->template_id));
 ...
 /**
  ...
  function addWikiEntries(){
      ...
      foreach($we_data as $id => $data){
          $result = db_query(sprintf("INSERT INTO plugin_phpwiki_group_list (group_id, wiki_name, wiki_link, description, rank, language_id)"
 ...
 /**
  ...
  function cloneWikiVersionTable($array){
      foreach($array as $key => $value){
	      ...
	      $result = db_query(sprintf("select version, mtime, minor_edit, content FROM plugin_phpwiki_version WHERE id=%d", $key));
	      while($row = db_fetch_array($result)){
	          $num_ver = $row['version'];
	          $res = db_query(sprintf("INSERT INTO plugin_phpwiki_version (id, version, mtime, minor_edit, content, versiondata)"
									  ."VALUES(%d, %d, %d, %d, '%s', '%s')"
 ...
 /**
  ...
  function cloneWikiPageTable(){
      $ids = array();
      $result = db_query(sprintf("SELECT id, pagename FROM plugin_phpwiki_page WHERE group_id=%d", $this->template_id));
      while($row = db_fetch_array($result)){
  ...
  function cloneWikiRecentTable($array){
	  ...
	  if (!empty($recent_infos)){
	      if(!empty($recent_infos['latestminor'])){
	          $result = db_query(sprintf("INSERT INTO plugin_phpwiki_recent (id, latestversion, latestmajor, latestminor)"
	                                    ."VALUES(%d, %d, %d, %d)", $new_id, $recent_infos['latestversion'], $recent_infos['latestmajor']
                                            , $recent_infos['latestminor']));
	      }else{ 
	          $result = db_query(sprintf("INSERT INTO plugin_phpwiki_recent (id, latestversion, latestmajor)"
 ...
 /**
  ...
  function addWikiLinkEntries($array){
      foreach($array as $tmpl_id => $new_id){
          $result = db_query(sprintf("select linkto FROM plugin_phpwiki_link WHERE linkfrom=%d", $tmpl_id));
          while($row = db_fetch_array($result)){
             // Find the new page target link
	     $clone_id = $this->getWikiPageCloneId($array, $row['linkto']);
	     // Insert a new link row in wiki_link table
	     $res = db_query(sprintf("INSERT INTO plugin_phpwiki_link (linkfrom, linkto) VALUES (%d, %d)", $new_id, $clone_id));
          }
      }
  }
 ...
 /**
  ...
  function cloneWikiAttachementTable(){
      ...
      $ids = array();
      $result = db_query(sprintf("SELECT id, name FROM plugin_phpwiki_attachment WHERE group_id=%d", $this->template_id));
      while($row = db_fetch_array($result)) {
          $id = $row['id'];
	  $name = $row['name'];
 ...
 /**
  ...
  function attachmentHasAPermission($attachment_id){
      $result = db_query(sprintf("SELECT count(*) AS nb FROM permissions WHERE permission_type='PHPWIKIATTACHMENT_READ' AND object_id=%d", $attachment_id));
  ...
  */
  function getAttachmentPermission($attachment_id){
      $result = db_query(sprintf("SELECT ugroup_id FROM permissions WHERE permission_type='PHPWIKIATTACHMENT_READ' AND object_id=%d", $attachment_id));
 ...
 /**
  ...
  function insertNewAttachmentPermission($new_attachment_id, $permission){
      $result = db_query(sprintf("INSERT INTO permissions (permission_type, object_id, ugroup_id)"
                                ."VALUES ('PHPWIKIATTACHMENT_READ', %d, %d)", $new_attachment_id, $permission));
  }
 ...
 /**
  ...
  function cloneWikiAttachmentRevisionTable($array){
      $array_rev = array();
      foreach($array as $tmpl_id => $new_id){
          $result = db_query(sprintf("SELECT user_id, date, revision, mimetype, size FROM plugin_phpwiki_attachment_revision WHERE attachment_id=%d", $tmpl_id));
          while($row = db_fetch_array($result)){
	      $res = db_query(sprintf("INSERT INTO plugin_phpwiki_attachment_revision (attachment_id, user_id, date, revision, mimetype, size)"
	      ...
	      if (db_affected_rows($res) > 0){
	          $sql = db_query(sprintf("SELECT id from plugin_phpwiki_attachment_revision WHERE attachment_id=%d AND revision=%d", $new_id, $row['revision']));
		  if(db_numrows($sql) > 0){
 ...
 /**
  ...
  function cloneAttachmentFileRevision($id, $revision_num){
       $result = db_query(sprintf("SELECT name from plugin_phpwiki_attachment where id=%d", $id));
 ...
 /**
  ...
  function cloneWikiAttachmentLogTable($array, $array_rev){
      foreach($array as $tmpl_id => $new_id){
          $result = db_query(sprintf("SELECT * FROM plugin_phpwiki_attachment_log WHERE group_id=%d AND wiki_attachment_id=%d", $this->template_id, $tmpl_id));
          while($row = db_fetch_array($result)){
              $res = db_query(sprintf("INSERT INTO plugin_phpwiki_attachment_log (user_id, group_id, wiki_attachment_id, wiki_attachment_revision_id, time)"
 ...
 /**
  ...
  function getTemplatePageRecentInfos($id){
      $recent = array();
      $result = db_query(sprintf("SELECT latestversion, latestmajor, latestminor FROM plugin_phpwiki_recent where id=%d", $id));
      if (db_numrows($result) > 0){
 ...
 /**
  ...
  function getTemplateWikiEntries(){
      $we = array();
      $result = db_query(sprintf("SELECT id, wiki_name, wiki_link, description, rank, language_id FROM plugin_phpwiki_group_list WHERE group_id=%d", $this->template_id));
      while ($row = db_fetch_array($result)){
	  $id = $row['id'];
 ...
 /**
  ...
  function getTemplateLanguageId(){
      $result = db_query(sprintf("SELECT language_id FROM plugin_phpwiki_group_list WHERE group_id=%d", $this->template_id));
 ...
 /**
  ...
  function getTemplatePageData($pagename){
      $result = db_query(sprintf("SELECT pagedata FROM plugin_phpwiki_page WHERE pagename='%s' AND group_id=%d", db_es($pagename), $this->template_id));
 ...
 /**
  ...
  function getTemplateWikiVersionData($id){
      $version = array();
      $result = db_query(sprintf("SELECT version, versiondata FROM plugin_phpwiki_version WHERE id=%d", $id));
      while($row = db_fetch_array($result)){
  ...
  function addNonEmptyInfo($array){
      foreach($array as $tmpl => $new){
          if ($this->isTemplatePageNonEmpty($tmpl)){
	       $result = db_query(sprintf("INSERT INTO plugin_phpwiki_nonempty (id) VALUES(%d)", $new));
	  }
      }
  }
 /**
  ...
  function insertNewAttachment($name){
      $result = db_query(sprintf("INSERT INTO plugin_phpwiki_attachment (group_id, name)"
				."VALUES(%d, '%s')", $this->group_id, $this->escapeString($name))); 
      if (!empty($result)){
          $res = db_query(sprintf("SELECT id FROM plugin_phpwiki_attachment WHERE group_id=%d AND name='%s'", $this->group_id, $this->escapeString($name)));
 ...
 /**
  ...
  function insertNewWikiPage($data, $pagename){
      $result = db_query(sprintf("INSERT INTO plugin_phpwiki_page (pagename, hits, pagedata, group_id)"
				 ."VALUES('%s', %d,  '%s', %d)"
 ...
 /**
  ...
  function cloneWikiPermission(){
      $result = db_query(sprintf("SELECT * FROM permissions where permission_type='PHPWIKI_READ' and object_id=%d", $this->template_id));
      while($row = db_fetch_array($result)){
          $res = db_query(sprintf("INSERT INTO permissions (permission_type, object_id, ugroup_id)"
	                             ."VALUES ('PHPWIKI_READ', %d, %d)", $this->group_id, $this->getMappedUGroupId($row['ugroup_id'])));
      }
  }
 ...
 /**
  ...
  function cloneWikiPagesPermissions($array){
      $result = db_query(sprintf("SELECT object_id, ugroup_id "
			                    ...
			                    ."WHERE perm.permission_type='PHPWIKIPAGE_READ' "
				                ."AND wpg.group_id=%d "
			                    ."AND perm.object_id=wpg.id", $this->template_id));

      while($row = db_fetch_array($result)){
          $res = db_query(sprintf("INSERT INTO permissions (permission_type, object_id, ugroup_id)"
	                             ."VALUES ('PHPWIKIPAGE_READ', %d, %d)", $this->getWikiPageCloneId($array, $row['object_id'])
