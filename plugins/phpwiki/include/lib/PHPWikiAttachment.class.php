...
require_once('PHPWikiAttachmentRevision.class.php');
require_once('PHPWikiAttachmentDao.class.php');
 ...
 */
class PHPWikiAttachment /* implements UGroupPermission */ {
    /**
     * Project identifier
     * @access private
     * @var    int 
    ...
    public function &getDao() {
        ...
        if(!$_codendi_wikiattachmentdao_instance) {
            $_codendi_wikiattachmentdao_instance = new PHPWikiAttachmentDao(CodendiDataAccess::instance());
        }
    ...
    public function setFilename($name = "") {
        ...
        if (preg_match("/[^._a-zA-Z0-9-\(\) &]/", $name)) {
            ...
            trigger_error($GLOBALS['Language']->getText('plugin_phpwiki_lib_attachment', 'err_alpha', array($name)), E_USER_ERROR);
        }
    ...
    public function dbadd() {
        ...
        if(!$created) {            
            trigger_error($GLOBALS['Language']->getText('plugin_phpwiki_lib_attachment',
                                                        'err_insert'),
                          E_USER_ERROR);
            return false;
        }
    ...
    public function create() {
        ...
        if(!is_dir($this->basedir)){
            ...
            if(!$res) {
                trigger_error($GLOBALS['Language']->getText('plugin_phpwiki_lib_attachment', 'err_create_upload_dir'), E_USER_ERROR);
                return false;
            }
        }
        ...
        if(!is_dir($this->basedir.'/'.$this->getFilesystemName())){
            ...
            if(!$res) {
                trigger_error($GLOBALS['Language']->getText('plugin_phpwiki_lib_attachment', 'err_create_file_dir'), E_USER_ERROR);
                return false;
            }
        }
    ...
    public function getId() {
        if(!is_numeric($this->id)) {
            ...
            if($dar->rowCount() > 1) {
                trigger_error($GLOBALS['Language']->getText('plugin_phpwiki_lib_attachment', 'err_multi_id'), E_USER_ERROR);
                return -1;
            } else {
                $row =& $dar->getRow();
                $this->id = $row['id'];
