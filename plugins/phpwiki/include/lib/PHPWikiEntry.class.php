...
class PHPWikiEntry {
  ...
  function setFromRow($row) {
    ...
    $this->wikiPage = new PHPWikiPage($this->gid, $this->page);
  }

  function _setFromDb() {
    $res = db_query(' SELECT * FROM plugin_phpwiki_group_list'.
  ...
  function getDesc() {
    ...
    function getEntryIterator($gid=null) {
        ...
        $qry = ' SELECT * FROM plugin_phpwiki_group_list'
   ...
   */
  function add() {
    $res = db_query(' INSERT INTO plugin_phpwiki_group_list SET'.
    ...
    if($res === false) {
      trigger_error($GLOBALS['Language']->getText('plugin_phpwiki_lib_wikientry',
				       'insert_err',
				       db_error()), 
		    E_USER_ERROR);
      return false;
  ...
  function del() {
    $res = db_query(' DELETE FROM plugin_phpwiki_group_list'.
		    ' WHERE id='.db_ei($this->id).
  ...
  function update() {
	...
	if(db_affected_rows() === 0) {
          $feedback .= $GLOBALS['Language']->getText('plugin_phpwiki_lib_wikientry',
					  'no_update',
					  $this->name);
	}
	return true;
