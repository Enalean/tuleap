...
class PHPWikiAttachmentDao extends DataAccessObject {
    /**
    ...
    function create($gid, $filename, $filesystemName) {
        $qry = sprintf(' INSERT INTO plugin_phpwiki_attachment (group_id, name, filesystem_name)'
                       .' VALUES (%d, %s, %s)',
                       $gid,
     ...
     */
    function read($id) {
        $qry = sprintf('SELECT * FROM plugin_phpwiki_attachment'
                       .' WHERE id=%d',
                       $id);
     ...
     */
    function delete($id) {
        $sql = 'UPDATE plugin_phpwiki_attachment SET delete_date='.$this->da->escapeInt($_SERVER['REQUEST_TIME']).
               ' WHERE id = '.$this->da->escapeInt($id);
        if ($this->update($sql)) {
            $sql = 'INSERT INTO plugin_phpwiki_attachment_deleted(id, group_id, name, delete_date)'.
    ...
    function getListWithCounterOrderedByRevDate($gid) {
        $qry = sprintf('SELECT wa.id, wa.group_id, wa.name, count(*) as nb, MAX(war.date) as max_date'
                       .' FROM plugin_phpwiki_attachment_revision AS war, plugin_phpwiki_attachment AS wa'
                       .' WHERE wa.group_id=%d'
    ...
    function getIdFromFilename($gid, $filename) {
        $qry = sprintf('SELECT id FROM plugin_phpwiki_attachment'
    ...
    function searchAttachmentToPurge($time, $groupId=0, $offset=0, $limit=0) {
        ...
        $sql = 'SELECT attachment.* '.
               ' FROM plugin_phpwiki_attachment_deleted attachment'.
    ...
    function restoreAttachment($id) {
        $sql = 'UPDATE plugin_phpwiki_attachment SET delete_date = NULL '.
                       'WHERE id = '.$this->da->escapeInt($id);
        if ($this->update($sql)) {
            $sql = 'DELETE FROM plugin_phpwiki_attachment_deleted WHERE id = '.$this->da->escapeInt($id);
    ...
    function setPurgeDate($id, $time) {
        $sql = 'UPDATE plugin_phpwiki_attachment_deleted SET purge_date ='.$this->da->escapeInt($time).
                       ' WHERE id = '.$this->da->escapeInt($id);
        return $this->update($sql);
    }
}
