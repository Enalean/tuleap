...
class PHPWiki_PermissionsManager {

    const WIKI_PERMISSION_READ         = 'PHPWIKIPAGE_READ';
    const SERVICE_WIKI_PERMISSION_READ = 'PHPWIKI_READ';
    ...
    private function filterWikiPagePermissionsAccordingToService(Project $project, array $wiki_page_ugroup_ids) {
        $plugin_phpwiki_service_ugroup_ids = $this->getWikiServicePermissions($project->getId());

        foreach($plugin_phpwiki_service_ugroup_ids as $plugin_phpwiki_service_ugroup_id) {
            $this->checkServiceOverridesPagePermission($wiki_page_ugroup_ids, $plugin_phpwiki_service_ugroup_id);
        }

        if (empty($wiki_page_ugroup_ids)) {
            $wiki_page_ugroup_ids = $plugin_phpwiki_service_ugroup_ids;
        }

        return array_merge($wiki_page_ugroup_ids, $this->getWikiAdminsGroups());
    }

    private function checkServiceOverridesPagePermission(array &$wiki_page_ugroup_ids, $plugin_phpwiki_service_ugroup_id) {
        foreach($wiki_page_ugroup_ids as $key => $wiki_page_ugroup_id) {
            ...
            $comparable_plugin_phpwiki_service_ugroup_id = $this->getComparableUGroupId($plugin_phpwiki_service_ugroup_id);

            if ((int)$comparable_plugin_phpwiki_service_ugroup_id > (int)$comparable_wiki_page_ugroup_id) {
