 ...
 */
class PHPWikiServiceViews extends PHPWikiViews {
  ...
  function PHPWikiServiceViews(&$controler, $id=0, $view=null) {
    ...
    if(isset($_REQUEST['pagename']) && !is_null($_REQUEST['pagename'])) {
        $this->html_params['title']  = $GLOBALS['Language']->getText('plugin_phpwiki_views_wikiserviceviews',
                                                          'wiki_page_title',
  ...
  function browsePages() {    
    ...
    print $GLOBALS['Language']->getText('plugin_phpwiki_views_wikiserviceviews', 'wiki_subtit_pages', array($hurl));
    if(!$hideFlag) {
    ...
    $hurl='<a href="'.$this->wikiLink.'&view=browsePages&'.$hideUrl.'">'.$hideImg.'</a>';
    print $GLOBALS['Language']->getText('plugin_phpwiki_views_wikiserviceviews', 'wiki_subtit_empty', array($hurl));
    if(!$hideFlag) {
    ...
    $hurl='<a href="'.$this->wikiLink.'&view=browsePages&'.$hideUrl.'">'.$hideImg.'</a>';
    print $GLOBALS['Language']->getText('plugin_phpwiki_views_wikiserviceviews', 'wiki_subtit_create', array($hurl));
    if(!$hideFlag) {
 ...
 /**
  ...
  function _browseWikiDocuments() {
    ...
    while($wei->valid()) {
      ...
      if(!empty($href)) {
          ...
          print $GLOBALS['Language']->getText('plugin_phpwiki_views_wikiserviceviews', 'wikientries', array($href, $description));
      }
  ...
  function _browsePages(&$pageList) {
    ...
    foreach($pageList as $pagename) {
      $wp = new PHPWikiPage($this->gid, $pagename);
  ...
  function _newPageForm($addr='') {
    ...
    <form name="newPage" method="post" action="'.$addr.'">
      ...
      <input type="hidden" name="group_id" value="'.$this->gid.'" />'.$GLOBALS['Language']->getText('plugin_phpwiki_views_wikiserviceviews', 'pagename').' <input type="text" name="name" value="" size="20" maxsize="255" />
   ...
   */
  function pagePerms() {
     $postUrl = PHPWIKI_PLUGIN_BASE_URL . '/index.php?group_id='.$this->gid.'&action=setWikiPagePerms';
   ...
   */
  function install() {
    echo $GLOBALS['Language']->getText('plugin_phpwiki_views_wikiserviceviews',
                            'install_intro', 
    ...
    echo '<form name="WikiCreation" method="post" action="'.$this->wikiLink.'">
             ...
             <input type="hidden" name="view" value="doinstall" />'.$GLOBALS['Language']->getText('plugin_phpwiki_views_wikiserviceviews', 'wiki_language').' ';
