...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    const group_id = Number.parseInt(getAttributeOrThrow(buttons_section, "data-group-id"), 10);
    const gitlab_server_uri = getAttributeOrThrow(buttons_section, "data-gitlab-uri");
    const gitlab_group_id = Number.parseInt(
        getAttributeOrThrow(buttons_section, "data-gitlab-group-id"),
        10,
    );
