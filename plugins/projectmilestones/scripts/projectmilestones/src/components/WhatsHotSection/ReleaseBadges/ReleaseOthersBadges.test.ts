...
describe("ReleaseOthersBadges", () => {
    function getPersonalWidgetInstance(
    ...
    ): VueWrapper<InstanceType<typeof ReleaseOthersBadges>> {
        release_data = {
            id: 2,
            capacity,
            total_sprint,
        ...
        return mount(ReleaseOthersBadges, component_options);
    }
    ...
    describe("Display points of capacity", () => {
        ...
        it("Given initial effort is bigger than capacity, Then the initial effort badge has a warning style", () => {
            initial_effort = 100;
            capacity = 50;
        ...
        it("Given initial effort is smaller than capacity, Then the initial effort badge has primary style", () => {
            capacity = 100;
            initial_effort = 50;
    ...
    it("Given user display widget, Then a good link to sprint planning is renderer", () => {
    ...
    it("When the user can't see the subplanning, Then he can't see the planning link", () => {
        const wrapper = getPersonalWidgetInstance(false);
        expect(wrapper.find("[data-test=planning-link]").exists()).toBe(false);
    });

    it("When there isn't sub-planning, Then there isn't any link to sub-planning", () => {
        resources = {
            milestones: {
                accept: {
