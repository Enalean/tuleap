...
class WorkflowManager {
    ...
    public function process(TrackerManager $engine, Codendi_Request $request, User $current_user) {
        if ($request->get('func') == 'admin-workflow-rules') {
            $action = new Tracker_Workflow_Action_Rules_EditRules($this->tracker, Tracker_FormElementFactory::instance(), new Tracker_Rule_Date_Factory(new Tracker_Rule_Date_Dao()));
