...
class Tracker_Migration_V3_FieldPermsDao extends DataAccessObject
{
    public function create($tv3_id, $tv5_id)
    {
        ...
        $sql = "INSERT INTO permissions(object_id, permission_type, ugroup_id)
                SELECT id, CONCAT('PLUGIN_', permission_type), ugroup_id
                FROM permissions, tracker_field
                WHERE tracker_id = $tv5_id
                  AND object_id = CONCAT($tv3_id, '#', old_id)
                  AND permission_type LIKE 'TRACKER_FIELD_%'";
        $this->update($sql);
    }
}
