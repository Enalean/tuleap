...
class TrackerRulesListValidator
{
    ...
    public function validateListRules(Tracker $tracker, array $value_field_list, array $list_rules): bool
    {
        ...
        foreach ($value_field_list as $field_id => $value) {
            ...
            if ($field) {
                $values[$field->getID()] = [
                    'field' => $field, 'values' => is_array($value) ? $value : [$value]
                ];
            }
        }
        ...
        foreach ($list_rules as $rule) {
            $dependencies = $this->getDependencies($rule);
        }
        ...
        foreach ($dependencies as $source => $not_used) {
            ...
            if (isset($values[$source])) {
                $error_occured = $this->checkFieldsValidityForAllDependencies($tracker, $dependencies, $values, $source, $value_field_list);
            }
        }
        return !$error_occured;
    }

    private function getDependencies(Tracker_Rule_List $rule): array
    {
        $dependencies = [];
    ...
    private function checkFieldsValidityForAllDependencies(Tracker $tracker, array $dependencies, array $values, int $source, array $value_field_list)
    {
        $error_occured = false;
        foreach ($dependencies[$source] as $target => $not_used_target) {
            ...
            if (isset($values[$target])) {
                $error_occured = $this->checkFieldsValidity($tracker, $values, $target, $source, $dependencies, $value_field_list);
            }
        }
    ...
    private function checkFieldsValidity(Tracker $tracker, array $values, int $target, int $source, $dependencies, array $value_field_list): bool
    {
        ...
        foreach ($target_values as $target_value) {
            ...
            if (!$valid) {
                $error_occured = true;
                $this->dealDependencesProblemsAndSendError($source, $target, $values, $target_value, $value_field_list);
            }
        }
    ...
    private function areRulesValid(Tracker $tracker, array $source_values, $target_value, array $dependencies, int $source, int $target): bool
    {
        $valid = false;
    ...
    private function dealDependencesProblemsAndSendError(int $source, int $target, array $values, $target_value, array $value_field_list)
    {
        ...
        $GLOBALS['Response']->addFeedback(Feedback::ERROR, $values[$source]['field']->getLabel() . '(' . implode(', ', $pb_source_values) . ') -> ' . $values[$target]['field']->getLabel() . '(' . implode(', ', $pb_target_values) . ')');
    }
