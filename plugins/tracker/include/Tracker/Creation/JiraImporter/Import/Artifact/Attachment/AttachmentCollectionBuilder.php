...
class AttachmentCollectionBuilder
{
     ...
     * @return Attachment[]
     */
    public function buildCollectionOfAttachment(IssueAPIRepresentation $issue_api_representation): array
    {
        $collection = [];
        if ($issue_api_representation->getFieldByKey('attachment') !== null) {
            foreach ($issue_api_representation->getFieldByKey('attachment') as $attachment_api_value) {
                $collection[] = Attachment::buildFromIssueAPIResponse($attachment_api_value);
            }
        }
