...
class ArtifactsInMonoTrackerXMLExporter
{
    ...
    private function getUrl(
        ...
        $issue_types_to_search = [];
        foreach ($jira_issue_types as $issue_type) {
            $issue_types_to_search[] = $issue_type->getId();
        }

        if (! empty($issue_types_to_search)) {
            $params = [
                'jql'    => 'project="' . $jira_project_id . '" AND issuetype in (' . implode(',', $issue_types_to_search) . ' )',
                'fields' => '*all',
