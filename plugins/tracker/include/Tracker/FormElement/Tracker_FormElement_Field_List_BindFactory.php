...
class Tracker_FormElement_Field_List_BindFactory // phpcs:ignore PSR1.Classes.ClassDeclaration.MissingNamespace,Squiz.Classes.ValidClassName.NotPascalCase
{
    ...
    public function getBind($field, $type)
    {
        ...
        $bind = new ListFieldNullBind($field);
        switch ($type) {
            case self::STATIK:
                ...
                if ($row = $dao->searchByFieldId($field->id)->getRow()) {
                    ...
                    $bind = new ListFieldStaticBind($field, $row['is_rank_alpha'], $values, $default_value, $decorators);
                }
                break;
            case self::USERS:
                $dao = new BindUsersDao();
                if ($row = $dao->searchByFieldId($field->id)->getRow()) {
                    $bind = new ListFieldUserBind($field, $row['value_function'], $default_value, $decorators);
                }
                break;
            case self::UGROUPS:
                $values = [];
                ...
                $bind = new ListFieldUserGroupBind($field, array_filter($values), $default_value, $decorators, $this->ugroup_manager, $this->getUgroupsValueDao());
                break;
            default:
    ...
    public function getInstanceFromRow($row)
    {
        switch ($row['type']) {
            ...
            case self::UGROUPS:
                return new ListFieldUserGroupBind(
                    $row['field'],
                    $row['values'],
                    $row['default_values'],
                    $row['decorators'],
            ...
            default:
                ...
                return new ListFieldNullBind($row['field']);
        }
    }
                ...
                if ($xml->items->item) {
                    foreach ($xml->items->item as $item) {
                        ...
                        if ($ugroup) {
                            $ID              = (string) $item['ID'];
                            $is_hidden       = isset($item['is_hidden']) && (int) $item['is_hidden'] ? 1 : 0;
                            $values[$ID]     = new ListFieldUserGroupBindValue($ID, $ugroup, $is_hidden);
                            $xmlMapping[$ID] = $values[$ID];
                        }
                    }
                }
        ...
        if (isset($xml->default_values)) {
            ...
            foreach ($xml->default_values->value as $default_value) {
                if (isset($default_value['REF'])) {
                    ...
                    $row['default_values'][$user->getId()] = new ListFieldUserBindValue(
                        $user->getId()
                    );
                }
            }
    ...
    {
        return new ListFieldStaticBindValue($id, $label, $description, $rank, $is_hidden);
    }

    /**
     * Build an instance of static value
     *
     * @return ListFieldUserGroupBindValue
     */
    private function getUgroupsValueInstance($id, Project $project, $ugroup_id, $is_hidden)
    {
        ...
        if ($ugroup) {
            return new ListFieldUserGroupBindValue($id, $ugroup, $is_hidden);
        }
    }
    ...
    public function fetchCreateABind($field)
    {
        ...
        $html .= '<dd class="tracker-bind-def">';
        $html .= ListFieldUserBind::fetchAdminCreateForm($field);
        $html .= '</dd>';
        ...
        $html .= '<dd class="tracker-bind-def">';
        $html .= ListFieldUserGroupBind::fetchAdminCreateForm($field);
        $html .= '</dd>';
        ...
        switch ($type) {
            ...
            case self::STATIK:
                ...
                if ($dao->save($field->getId(), 0)) {
                    $bind = new ListFieldStaticBind($field, 0, [], [], []);
            ...
            case self::USERS:
                ...
                if ($dao->save($field->getId(), [])) {
                    $bind = new ListFieldUserBind($field, '', [], []);
                    $bind->process($bind_data, 'no redirect');
                }
                break;
            case self::UGROUPS:
                $bind = new ListFieldUserGroupBind($field, [], [], [], $this->ugroup_manager, $this->getUgroupsValueDao());
                $bind->process($bind_data, 'no redirect');
                break;
            default:
                break;
    ...
    public function getType($bind)
    {
        return $bind instanceof \Tuleap\Tracker\FormElement\Field\List\Bind\Static\ListFieldStaticBind ? self::STATIK :
                ($bind instanceof \Tuleap\Tracker\FormElement\Field\List\Bind\User\ListFieldUserBind ? self::USERS :
                    ($bind instanceof \Tuleap\Tracker\FormElement\Field\List\Bind\UserGroup\ListFieldUserGroupBind ? self::UGROUPS : '')
                );
    }
