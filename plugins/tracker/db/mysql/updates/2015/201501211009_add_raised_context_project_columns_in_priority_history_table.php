...
class b201501211009_add_raised_context_project_columns_in_priority_history_table extends \Tuleap\ForgeUpgrade\Bucket
{
    ...
    public function postUp()
    {
        if (! $this->db->columnNameExists('tracker_artifact_priority_history', 'has_been_raised')) {
            throw new \Tuleap\ForgeUpgrade\Bucket\BucketUpgradeNotCompleteException('An error occured while adding has_been_raised column to tracker_artifact_priority_history table');
        }

        if (! $this->db->columnNameExists('tracker_artifact_priority_history', 'context')) {
            throw new \Tuleap\ForgeUpgrade\Bucket\BucketUpgradeNotCompleteException('An error occured while adding context column to tracker_artifact_priority_history table');
        }

        if (! $this->db->columnNameExists('tracker_artifact_priority_history', 'project_id')) {
            throw new \Tuleap\ForgeUpgrade\Bucket\BucketUpgradeNotCompleteException('An error occured while adding project_id column to tracker_artifact_priority_history table');
        }
    }
}
