...
codendi.tracker.bind.Editor = Class.create({
    ...
    editStaticValues: function (element) {
        ...
        var link = new Element('a', { href: '#', title: 'Edit ' + tf_label.value }).update(tuleap.escaper.html(tf_label.value));
        var descr = new Element('div').addClassName('tracker-admin-bindvalue_description').update(
            this.nl2br(tuleap.escaper.html(this.wordwrap(ta_description.value, 80, '\n')))
        );
