...
class CollectionOfSemanticsUsingAParticularTrackerFieldTest extends \Tuleap\Test\PHPUnit\TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->project->method('getID')->willReturn(140);
    }
    ...
    public function testItReturnsUsages(): void
    {
        ...
        $tracker_from_same_project->method('getId')->willReturn(11);
        $tracker_from_same_project->method('getProject')->willReturn($this->project);
        $tracker_from_same_project->method('getName')->willReturn('User stories');

        $another_project = $this->createMock(\Project::class);
        $another_project->method('getID')->willReturn(150);
        $another_project->method('getPublicName')->willReturn('Project X');

        $tracker_from_another_project = $this->createMock(\Tuleap\Tracker\Tracker::class);
        $tracker_from_another_project->method('getId')->willReturn(12);
        $tracker_from_another_project->method('getProject')->willReturn($another_project);
        $tracker_from_another_project->method('getName')->willReturn('Sprints');
    ...
    private function getMockedSemantic(string $semantic_label, \Tuleap\Tracker\Tracker $tracker)
    {
        ...
        $semantic->method('getLabel')->willReturn($semantic_label);
        $semantic->method('getTracker')->willReturn($tracker);
