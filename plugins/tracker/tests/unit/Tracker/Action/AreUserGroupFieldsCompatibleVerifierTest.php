...
final class AreUserGroupFieldsCompatibleVerifierTest extends \Tuleap\Test\PHPUnit\TestCase
{
    ...
    public function testItReturnsFalseWhenTheLastChangesetIsNotAUserGroupListValueChangeset(): void
    {
        $verifier = new AreUserGroupFieldsCompatibleVerifier();
        ...
        self::assertFalse(
            $verifier->areUserGroupFieldsCompatible($this->source_field, $this->target_field, $this->artifact)
        );
    }
    ...
    public function testItReturnsFalseWhenTheSourceAndDestinationFieldsDoNotHaveTheSameMultiplicity(): void
    {
        ...
        $last_changeset_value_value = $this->createStub(\Tracker_FormElement_Field_List_Bind_UgroupsValue::class);
        $last_changeset_value       = $this->createStub(Tracker_Artifact_ChangesetValue_List::class);
        $last_changeset_value->method('getListValues')->willReturn([$last_changeset_value_value]);
        ...
        self::assertFalse($verifier->areUserGroupFieldsCompatible($source_single, $target_multiple, $this->artifact));
        self::assertFalse($verifier->areUserGroupFieldsCompatible($source_multiple, $target_single, $this->artifact));
        self::assertTrue($verifier->areUserGroupFieldsCompatible($source_single, $target_single, $this->artifact));
        self::assertTrue($verifier->areUserGroupFieldsCompatible($source_multiple, $target_multiple, $this->artifact));
    }
    ...
    public function testReturnsFalseWhenThereIsNoMatchingValueInDestinationTracker(): void
    {
        ...
        self::assertFalse(
            $verifier->areUserGroupFieldsCompatible(
                $this->source_field,
                $this->target_field,
                $this->artifact
            )
    ...
    public function testReturnsTrueWhenBindIsUserGroupAndThereIsAMatchingValueInDestinationTracker(): void
    {
        $verifier = new AreUserGroupFieldsCompatibleVerifier();
