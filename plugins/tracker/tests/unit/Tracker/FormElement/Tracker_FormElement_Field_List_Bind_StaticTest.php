...
class Tracker_FormElement_Field_List_Bind_StaticTest extends TestCase
{
    ...
    public function testNoneValueCanNotBeHidden(): void
    {
        $dao = Mockery::mock(Tracker_FormElement_Field_List_Bind_Static_ValueDao::class);
        $bind = Mockery::mock(Tracker_FormElement_Field_List_Bind_Static::class)->makePartial()->shouldAllowMockingProtectedMethods();

        $value = Mockery::mock(Tracker_FormElement_Field_List_Bind_StaticValue::class);
        $value->shouldReceive('getId')->andReturn(\Tracker_FormElement_Field_List::NONE_VALUE);

        $bind->shouldReceive('getValueDao')->andReturn($dao);

        $this->assertFalse($bind->canValueBeHidden($value));
    }

    public function testValueCanBeHidden(): void
    {
        $dao = Mockery::mock(Tracker_FormElement_Field_List_Bind_Static_ValueDao::class);
        $field = Mockery::mock(\Tracker_FormElement_Field_List::class);
        $bind = Mockery::mock(Tracker_FormElement_Field_List_Bind_Static::class, [$field, null, null, null, null])->makePartial()->shouldAllowMockingProtectedMethods();

        $value = Mockery::mock(Tracker_FormElement_Field_List_Bind_StaticValue::class);
        $value->shouldReceive('getId')->andReturn(12);

        $bind->shouldReceive('getValueDao')->andReturn($dao);
        $dao->shouldReceive('canValueBeHidden')->andReturnTrue();

        $this->assertTrue($bind->canValueBeHidden($value));
    }

    public function testNoneValueCanNotBeDeleted(): void
    {
        $dao = Mockery::mock(Tracker_FormElement_Field_List_Bind_Static_ValueDao::class);
        $bind = Mockery::mock(Tracker_FormElement_Field_List_Bind_Static::class)->makePartial()->shouldAllowMockingProtectedMethods();
        ...
        $this->assertFalse($bind->canValueBeDeleted($value));
    }

    public function testValueCanBeDeleted(): void
    {
        $dao = Mockery::mock(Tracker_FormElement_Field_List_Bind_Static_ValueDao::class);
        $field = Mockery::mock(\Tracker_FormElement_Field_List::class);
        $bind = Mockery::mock(Tracker_FormElement_Field_List_Bind_Static::class, [$field, null, null, null, null])->makePartial()->shouldAllowMockingProtectedMethods();
        ...
        $bind->shouldReceive('getValueDao')->andReturn($dao);
        $dao->shouldReceive('canValueBeDeleted')->andReturnTrue();

        $this->assertTrue($bind->canValueBeDeleted($value));
    }
}
