...
{
    private Tracker_Artifact_ChangesetValue_Integer&MockObject $old_value;
    private ChangesChecker $checker;

    protected function setUp(): void
    {
        $this->old_value = $this->createMock(Tracker_Artifact_ChangesetValue_Integer::class);
        $this->checker   = new ChangesChecker();
    }

    public function testChecksIfChangesOccuredAtArtifactUpdate(): void
    {
        $this->old_value->method('getNumeric')->willReturn(1);

        self::assertTrue($this->checker->hasChanges($this->old_value, 2));
        self::assertFalse($this->checker->hasChanges($this->old_value, 1));
    }
    ...
    {
        $this->old_value->method('getNumeric')->willReturn(0);
        $new_value = null;
