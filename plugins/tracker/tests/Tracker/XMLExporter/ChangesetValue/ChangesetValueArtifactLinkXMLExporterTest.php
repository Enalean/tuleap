...
class Tracker_XMLExporter_ChangesetValue_ChangesetValueArtifactLinkXMLExporterTest extends TuleapTestCase {
    ...
    public function setUp() {
        ...
        $this->exporter      = new Tracker_XMLExporter_ChangesetValue_ChangesetValueArtifactLinkXMLExporter(
            $this->collector
        );
    ...
    public function itExportsChildren() {
        stub($this->changeset_value)->getValue()->returns(array(
            new Tracker_ArtifactLinkInfo(111, '*', '*', 101, '*'),
            new Tracker_ArtifactLinkInfo(222, '*', '*', 102, '*'),
        ));
    ...
    public function itDoesNotExportArtifactsThatAreNotChildren() {
        stub($this->changeset_value)->getArtifactIds()->returns(array(
            new Tracker_ArtifactLinkInfo(333, '*', '*', 103, '*')
        ));
    ...
    public function itCollectsParentAsWell() {
        $artifact_id = 123;
        $artifact    = anArtifact()->withId($artifact_id)->build();
        ...
        $this->assertEqual($this->collector->getAllParents(), array($artifact_id));
    }
}