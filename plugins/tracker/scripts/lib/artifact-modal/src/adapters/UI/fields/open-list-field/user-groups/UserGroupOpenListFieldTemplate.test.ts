...
import type { HostElement } from "./UserGroupOpenListField";
...
describe("UserGroupOpenListFieldTemplate", () => {
    ...
    const getRenderedStaticOpenListField = (
        ...
        const render = renderUserGroupOpenListField(host);
        render(host, target);

        return target;
    };

    it("should render a user group open list field with its values", () => {
        const presenter = {
            field_id: "tracker_field_1001",
            required: true,
            label: "User groups open list field",
            hint: "Please select some user groups in the list",
            name: "user_group_open_list_field",
    ...
    it(`When the field is disabled, then:
        ...
        const field_element = getRenderedStaticOpenListField({
            values: [],
        } as unknown as UserGroupOpenListFieldPresenter);
        const form_element = selectOrThrow(field_element, "[data-test=openlist-field]");
        const select_element = selectOrThrow(
            field_element,
            "[data-test=user-group-open-list-field-select]",
    ...
    it(`When the field is required, and no value has been selected, then
        ...
        const error_element = selectOrThrow(
            field_element,
            "[data-test=user-group-open-list-field-error]",
        );
