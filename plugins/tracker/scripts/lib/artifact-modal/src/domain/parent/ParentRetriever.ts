...
import { NoParentFault } from "./NoParentFault";
import { ParentRetrievalFault } from "./ParentRetrievalFault";
...
export const ParentRetriever = (retriever: RetrieveArtifact): RetrieveParent => ({
    getParent: (
        ...
        if (parent_identifier === null) {
            return errAsync(NoParentFault());
        }
        return retriever.getArtifact(parent_identifier).mapErr(ParentRetrievalFault);
    },
});
