...
describe(`FloatField`, () => {
    let dispatchEvent: jest.SpyInstance, host: Element & FloatField, inner_input: HTMLInputElement;
    beforeEach(() => {
        ...
        host = {
            ...
            value: 0,
            dispatchEvent,
        } as unknown as Element & FloatField;
    ...
    it(`when the input's value is a number, it dispatches a "value-changed" event with the number`, () => {
        inner_input.value = "13.5";
        inner_input.dispatchEvent(new InputEvent("input"));

        const event = dispatchEvent.mock.calls[0][0];
        expect(event.type).toEqual("value-changed");
        expect(event.detail.field_id).toEqual(87);
        expect(event.detail.value).toEqual(13.5);
    });
