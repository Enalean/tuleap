...
final class ManagerIsSeeingTheirOwnTimeVerifierTest extends VerifierChainTestBase
{
    public function testTrueIfSameUser(): void
    {
        ...
        $time = new TimeSpentInArtifact($manager, $artifact, 123);

        $next_in_chain = VerifyInChainManagerIsAllowedToSeeTimesStub::withoutAllowed();

        $verifier = new ManagerIsSeeingTheirOwnTimeVerifier();
    ...
    public function testDelegatesToNextInChainIfDifferentUser(): void
    {
        $artifact = ArtifactTestBuilder::anArtifact(101)->build();
        ...
        $this->assertNextInChainDelegation(
            static fn () => new ManagerIsSeeingTheirOwnTimeVerifier(),
            $manager,
            $time,
        );
    }
