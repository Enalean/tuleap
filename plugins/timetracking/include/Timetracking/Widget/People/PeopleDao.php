...
use Tuleap\Timetracking\REST\v1\PeopleTimetracking\SearchUsersByWidgetId;
use Tuleap\Timetracking\REST\v1\PeopleTimetracking\UserList;

final class PeopleDao extends DataAccessObject implements SaveQueryWithDates, SaveQueryWithPredefinedTimePeriod, GetQueryUsers, GetWidgetInformation, SearchQueryByWidgetId, SearchUsersByWidgetId
{
    public function create(PredefinedTimePeriod $predefined_time_period): int
    {
        $this->getDB()->run('INSERT INTO plugin_timetracking_people_query (predefined_time_period)
    ...
    public function delete(int $query_id): void
    {
        $this->getDB()->run(
            <<<EOS
            DELETE plugin_timetracking_people_query, plugin_timetracking_people_query_users
            FROM plugin_timetracking_people_query
                INNER JOIN plugin_timetracking_people_query_users ON (id = query_id)
            WHERE id = ?
            EOS,
            $query_id,
        );
    ...
    public function saveQueryWithDates(
        ...
        $this->getDB()->tryFlatTransaction(function () use ($query_id, $start_date, $end_date, $users) {
            $sql = 'UPDATE plugin_timetracking_people_query
    ...
    {
        $sql = 'DELETE FROM plugin_timetracking_people_query_users WHERE query_id = ? ';
    ...
    private function insertUsers(int $query_id, UserList $users): void
    {
        ...
        if ($users_to_insert !== []) {
            $this->getDB()->insertMany('plugin_timetracking_people_query_users', $users_to_insert);
        }
    }
    ...
    {
        $sql = 'SELECT user_id
                FROM plugin_timetracking_people_query_users
                WHERE query_id = ?';
        ...
        $sql = 'SELECT dashboard_id, user_id
                FROM plugin_timetracking_people_query
                         INNER JOIN dashboards_lines_columns_widgets AS widget
                                    ON plugin_timetracking_people_query.id = widget.content_id
                ...
                WHERE plugin_timetracking_people_query.id = ?
                  AND widget.name = "people-timetracking-widget"
