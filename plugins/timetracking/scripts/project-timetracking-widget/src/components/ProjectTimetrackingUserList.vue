...
<template>
    <div class="tlp-form-element">
        <div class="tlp-form-element tlp-form-element-append">
            <select
                class="project-timetracking-users-selector-input tlp-select tlp-select-small"
                id="tracker"
                v-on:input="setSelected"
                data-test="project-timetracking-users-selector"
            >
                <option v-bind:value="null">{{ $gettext("All users") }}</option>
                <option
                    v-for="user in project_timetracking_store.users"
...
function setSelected(event: Event): void {
    ...
    project_timetracking_store.setSelectedUserId(Number.parseInt(event.target.value, 10));
}
</script>
