...
describe("ProjectTimetrackingTrackerList tests", () => {
    describe("Given a project timetracking widget on reading mode", () => {
        const getWrapper = (selected_trackers: ProjectReportTracker[]): VueWrapper => {
            ...
            return shallowMount(ProjectTimetrackingTrackerList, {
        ...
        it("When no selected trackers, then 'no trackers selected' is displayed", () => {
            ...
            expect(
                wrapper
                    .find("[data-test=project-timetracking-reading-mode-trackers-empty]")
                    .exists(),
            ).toBe(true);
        });

        it("When trackers are selected, then empty field is not displayed", () => {
            const selected_trackers: ProjectReportTracker[] = [
                {
                    id: 16,
                    label: "tracker",
