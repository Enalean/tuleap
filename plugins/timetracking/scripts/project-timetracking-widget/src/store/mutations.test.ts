...
import type {
    ProjectReportTracker,
    TrackerWithTimes,
} from "@tuleap/plugin-timetracking-rest-api-types";
import { useProjectTimetrackingWidgetTestStore } from "../../tests/helpers/pinia-test-store";
import type { ProjectTimetrackingWidgetStoreInstance } from "../../tests/helpers/pinia-test-store";
import type { ProjectTracker } from "./state";

describe("Store mutations", (): void => {
    let store: ProjectTimetrackingWidgetStoreInstance;

    beforeEach((): void => {
        setActivePinia(createPinia());
        store = useProjectTimetrackingWidgetTestStore();
    });
    ...
    describe("Given a widget with state initialisation", (): void => {
        it("When selected trackers are set, state must change too", (): void => {
            const trackers = [{ id: 1, label: "timetracking_tracker" } as ProjectReportTracker];
        ...
        it("When trackers id are set, state must change too", (): void => {
            const trackers = [
                { id: 1, label: "timetracking_tracker" } as ProjectReportTracker,
                { id: 2, label: "support_tracker" } as ProjectReportTracker,
            ];
        ...
        describe("When trackers are added, state must change too", (): void => {
            beforeEach(() => {
                const trackers = [
                    { id: 1, label: "timetracking_tracker" } as ProjectReportTracker,
                    { id: 2, label: "support_tracker" } as ProjectReportTracker,
                    { id: 3, label: "task_tracker" } as ProjectReportTracker,
                ];
        ...
        describe("When trackers are set, state must change too", (): void => {
            beforeEach(() => {
                const trackers: ProjectReportTracker[] = [];
                store.setSelectedTrackers(trackers);
            });
