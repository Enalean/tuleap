...
import type { ProjectTimetrackingWidgetState } from "./state";
import {
...
export type ProjectTimetrackingWidgetStoreActions = {
    ...
    setSelectedTrackers(trackers: ProjectReportTracker[]): void;
    ...
    setTrackers(trackers: ProjectReportTracker[]): void;
    setTrackersIds(): void;
    setSelectedUserId(user: number): void;
    addSelectedTrackers(tracker_id: number): void;
    setIsReportSave(is_report_saved: boolean): void;
    removeSelectedTracker(tracker: ProjectReportTracker): void;
    setReportId(report_id: number): void;
    setUsers(time: TrackerWithTimes): void;
};

export type ProjectTimetrackingWidgetStoreGetters = {
    has_error(state: ProjectTimetrackingWidgetState): boolean;
    has_success_message(state: ProjectTimetrackingWidgetState): boolean;
    can_results_be_displayed(state: ProjectTimetrackingWidgetState): boolean;
    get_formatted_total_sum(state: ProjectTimetrackingWidgetState): string;
    is_sum_of_times_equals_zero(state: ProjectTimetrackingWidgetState): boolean;
    is_tracker_total_sum_equals_zero(
        state: ProjectTimetrackingWidgetState,
    ): (time_per_user: UserTotalTrackerTimes[]) => boolean;
    get_formatted_time(state: ProjectTimetrackingWidgetState): (times: TrackerWithTimes) => string;
};

export type ProjectTimetrackingWidgetStoreDefinition = StoreDefinition<
    string,
...
const getTotalSum = (state: ProjectTimetrackingWidgetState): number => {
...
/**
 * Define a store for one instance of a ProjectTimetrackingWidget. It is named after the report of the widget.
 */
export function useProjectTimetrackingWidgetStore(
    report_id: number,
): ProjectTimetrackingWidgetStoreDefinition {
    return defineStore(`project-timetracking/${report_id}`, {
        state: (): ProjectTimetrackingWidgetState => ({ ...default_state }),
        getters: {
            has_error: (state: ProjectTimetrackingWidgetState): boolean =>
