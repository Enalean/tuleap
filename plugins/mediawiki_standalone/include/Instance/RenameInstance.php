...
final class RenameInstance implements InstanceOperation
{
    ...
    public static function fromEvent(WorkerEvent $event, ProjectByIDFactory $project_factory): ?self
    {
        ...
        if (! isset($payload['new_name']) || ! is_string($payload['new_name'])) {
            throw new \Exception(sprintf('Payload doesnt have new_name or new_name is not string: %s', var_export($payload, true)));
        }
    ...
    public function getRequest(RequestFactoryInterface $request_factory, StreamFactoryInterface $stream_factory): RequestInterface
    {
        return $request_factory->createRequest(
            'POST',
            sprintf('%s/mediawiki/w/rest.php/tuleap/instance/rename/%s/%s', ServerHostname::HTTPSUrl(), urlencode($this->project->getUnixNameLowerCase()), urlencode($this->new_name))
        );
    }
