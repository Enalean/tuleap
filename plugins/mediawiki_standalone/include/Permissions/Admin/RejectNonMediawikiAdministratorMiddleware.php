...
use Tuleap\MediawikiStandalone\Permissions\IBuildUserPermissions;
use Tuleap\Request\ForbiddenException;
use Tuleap\User\ProvideCurrentUser;

final class RejectNonMediawikiAdministratorMiddleware implements \Psr\Http\Server\MiddlewareInterface
{
    public function __construct(
        private ProvideCurrentUser $current_user_provider,
        private IBuildUserPermissions $user_permissions_builder,
    ) {
    }
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        ...
        if (! $this->user_permissions_builder->getPermissions($user, $project)->is_admin) {
