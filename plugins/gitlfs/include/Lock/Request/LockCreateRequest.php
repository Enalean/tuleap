...
class LockCreateRequest implements GitLfsHTTPOperation
{
    ...
    /**
     * @throws IncorrectlyFormattedLockCreateRequestException
     * @return self
     */
    public static function buildFromJSONString(string $json_string) : LockCreateRequest
    {
        ...
        if ($json_decode_error_code !== JSON_ERROR_NONE) {
            throw new IncorrectlyFormattedLockCreateRequestException('JSON is not valid: ' . json_last_error_msg());
        }
    ...
    private static function buildFromObject(\stdClass $parameters) : LockCreateRequest
    {
        if (! isset($parameters->path)) {
            throw new IncorrectlyFormattedLockCreateRequestException('path should be present in the request');
        }

        $reference = null;
        if (isset($parameters->ref)) {
            if (! isset($parameters->ref->name)) {
                throw new IncorrectlyFormattedLockCreateRequestException(
