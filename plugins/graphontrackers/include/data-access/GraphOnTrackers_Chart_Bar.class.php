...
class GraphOnTrackers_Chart_Bar extends GraphOnTrackers_Chart {
    ...
    function userCanVisualize(){
        ...
        if($this->field_group){
            ...
            if(!$artifact_field_group->userCanRead($GLOBALS['group_id'],$GLOBALS['ath']->getID(), $user_id)){
                return false;
            }
        }
            
        if($artifact_field_base->userCanRead($GLOBALS['group_id'],$GLOBALS['ath']->getID(), $user_id)){
            return true;
        }else{return false;}
    }
