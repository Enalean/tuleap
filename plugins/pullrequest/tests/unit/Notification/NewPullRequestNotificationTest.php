...
final class NewPullRequestNotificationTest extends TestCase
{
    ...
    public function testItBuildsANotificationToProcess(): void
    {
        ...
        $owner       = UserTestBuilder::anActiveUser()->withId(101)->withUserName('alice')->build();
        $user_helper = $this->createMock(UserHelper::class);
        $user_helper->method('getDisplayNameFromUser')->with($owner)->willReturn('alice');
        $user_helper->method('getAbsoluteUserURL')->with($owner)->willReturn('https://example.com/alice');
        ...
        $notification = NewPullRequestNotification::fromPullRequest(
            $pull_request,
            GitRepositoryTestBuilder::aProjectRepository()->build(),
            new MentionedUserCollection([$hector, $owner]),
        ...
        self::assertSame('alice created the pull request #25: feat: Add some features', $notification->asPlaintext());
        self::assertSame(
            <<<EOF
            <p>
            <a href="https://example.com/alice">alice</a> created the pull request <a href="https://example.com/pr25">#25</a>: feat: Add some features</p>
            <p>
