...
export const buildCommitsTabUrl = (base_url: URL): URL => {
    const commits_base_url = new URL(base_url);
    commits_base_url.searchParams.set("tab", "commits");

    return commits_base_url;
};

export const buildFilesTabUrl = (base_url: URL, pull_request_id: number): string => {
    const files_tab_url = new URL("", base_url.toString());
    files_tab_url.hash = `#/pull-requests/${pull_request_id}/files`;

    return files_tab_url.toString();
};
