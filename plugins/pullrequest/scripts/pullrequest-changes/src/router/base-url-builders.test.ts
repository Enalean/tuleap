...
describe("base-url-builders", () => {
    ...
    it("should build the commits app url", () => {
        ...
        expect(buildCommitsTabUrl(base_url, pull_request_id).toString()).toBe(
            `https://example.com/plugins/git/?action=pull-requests&repo_id=${repository_id}&group_id=${project_id}&tab=commits#/pull-requests/${pull_request_id}/commits`,
        );
    });
    ...
    it("should build the files changes app url", () => {
        ...
        expect(buildChangesTabUrl(base_url).toString()).toBe(
            `https://example.com/plugins/git/?action=pull-requests&repo_id=${repository_id}&group_id=${project_id}&tab=changes`,
        );
    });
});
