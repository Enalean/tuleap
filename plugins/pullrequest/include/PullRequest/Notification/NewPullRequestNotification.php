...
final readonly class NewPullRequestNotification implements NotificationToProcess
{
    /**
    ...
    public static function fromPullRequest(
        ...
        $owner_display_name = $user_helper->getDisplayNameFromUser($owner);
        return new self(
            $pull_request,
            $filter_user_from_collection->filter($owner, ...$mentioned_user_collection->users),
            sprintf(
                dgettext('tuleap-pullrequest', '%s created the pull request #%d: %s'),
                $owner_display_name,
                $pull_request->getId(),
            ...
            new NotificationTemplatedContent(
                ...
                'new-pull-request-mail-content',
                new NewPullRequestMailContentPresenter(
                    $user_helper->getAbsoluteUserURL($owner),
                    $owner_display_name,
