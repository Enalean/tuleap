...
/**
 * @template-implements NotificationToProcessBuilder<PullRequestDescriptionUpdatedEvent>
 */
final readonly class PullRequestDescriptionUpdatedNotificationToProcessBuilder implements NotificationToProcessBuilder
{
    ...
    public function getNotificationsToProcess(EventSubjectToNotification $event): array
    {
        return $this->pull_request_retriever->getPullRequestById($event->getPullRequestId())->match(
            function (PullRequest $pull_request) use ($event) {
                $updater = $this->user_retriever->getUserById($event->getUserId());
                if ($updater === null) {
                    return [];
                }
                ...
                return [
                    PullRequestDescriptionUpdatedNotification::fromPullRequest(
                        $pull_request,
                        $repository,
