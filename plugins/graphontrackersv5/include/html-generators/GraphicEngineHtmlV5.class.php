...
class graphicEngineHtml extends Error {
    ...
    function showAvailableReports() {
        ...
        echo '<H2>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','tracker').
             ' \'<a href="/tracker/admin/?group_id='.$group_id.'&atid='.$atid.'">'.$hp->purify($GLOBALS['ath']->getName()).'</a>\''.
             $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','report_admin').
             '</H2>';

        if ($reports) {
            // Loop through the list of all graphic reports
            $title_arr = array();
            $title_arr[] = $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','report_name');
            $title_arr[] = $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','desc');
            $title_arr[] = $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','scope');
            $title_arr[] = $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','delete');

            echo '<p>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','mod');
            echo  html_build_list_table_top ($title_arr);

            for ($i=0;$i<count($reports);$i++) {
                $r = new GraphOnTrackersV5_Report($reports[$i]);
                ...
                if ( $r->getScope() == 'S' || (!$GLOBALS['ath']->userIsAdmin()&&($r->getScope() == 'P')) ) {
                    ...
                    echo  '<A HREF="/tracker/admin/?func=reportgraphic'.'&report_graphic_id='.$r->getId().'&group_id='.$group_id.
                          ...
                          '" onClick="return confirm(\''.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','delete_report',$r->getName()).'\')">'.
            ...
            echo  '<p><h3>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','no_rep_def').'</h3>';
        }
        echo  '<P> '.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','create_report',array('/tracker/admin/?func=reportgraphic&group_id='.$group_id.'&atid='.$atid.'&new_report_graphic=1'));
    }
    ...
    function createReportForm() {
        ...
        echo '<H2>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','tracker').
             ...
             $GLOBALS['ath']->getName().'</a>\'  - '.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','create_rep').
             ' </H2>';
        ...
        echo '<FORM NAME="create_rep_graphic" ACTION="/tracker/admin/" METHOD="POST">
              ...
              <B>'.$hp->purify($GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','name')).':</B>
              </td><td>
              <INPUT TYPE="TEXT" NAME="rep_name" VALUE="" SIZE="20" MAXLENGTH="20">
              &nbsp;&nbsp;&nbsp;&nbsp;
              <B>'.$hp->purify($GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','scope')).': </B>';
        ...
        echo '</td><td rowspan="2"  valign="middle"><input type="submit" name="submit" value="'. $GLOBALS['Language']->getText('global', 'btn_submit') .'" /></td></tr>
              <tr valign="top"><td>
               <B>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','desc').': </B></td><td>
    ...
    function showReportForm($report_graphic_id) {
        ...
        echo '<H2>'.
                $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','tracker').
                ' \'<A href="/tracker/admin/?group_id='.$group_id.'&atid='.$atid.'">'.
                    $hp->purify($GLOBALS['ath']->getName()).
                '</A>\' -  '.
                $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','modify_report').' \''.$hp->purify($gr->name).'\' ';
		echo help_button('TrackerAdministration.html#GraphTrackerReportSetting').'</H2>';
        echo '<p><a href="/tracker/admin/?func=reportgraphic&amp;group_id='. (int)$group_id .'&amp;atid='. (int)$atid .'">&laquo; '. $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','return_reports') .'</a></p>';
        echo '<FORM NAME="show_rep_graphic" ACTION="/tracker/admin/" METHOD="POST">
              ...
              <INPUT TYPE="HIDDEN" NAME="post_changes_graphic" VALUE="y">
              <table><tr valign="top"><td>
              <B>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','name').': </B>
              </td><td>
              <INPUT TYPE="TEXT" NAME="rep_name" VALUE="'.$hp->purify($gr->getName()).'" SIZE="20" MAXLENGTH="20">
              &nbsp;&nbsp;&nbsp;&nbsp;
              <B>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','scope').': </B>';
        ...
        echo '</td><td rowspan="2"  valign="middle"><input type="submit" name="update_report" value="'. $GLOBALS['Language']->getText('global', 'btn_submit') .'" /></td></tr>
              <tr valign="top"><td><B>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','desc').':</B></td><td>
              <INPUT TYPE="TEXT" NAME="rep_desc" VALUE="'.$hp->purify($gr->getDescription()).'" class="textfield_medium" />
              </td></tr></table>';

        echo '<hr /><p><strong>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','add_chart').'</strong> ';
        ...
        foreach($gr->getCharts() as $chart) {
            ...
            echo '<a title="'. $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report', 'tooltip_edit') .'" href="'. $url .'&amp;edit_chart='. $chart->getId() .'"><img src="'. util_get_dir_image_theme() .'ic/edit.png" alt="edit" /></a>';
            echo '<input title="'. $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report', 'tooltip_del') .'" type="image" src="'. util_get_dir_image_theme() .'ic/cross.png" onclick="return confirm('.$GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','confirm_del').');" name="delete_chart['. $chart->getId() .']" />';
            $chart->display();
            echo '</div>';
        }
    ...
    function showChartForm($chart) {
        ...
        echo '<H2>'.
                ...
                ' \'<A href="/tracker/admin/?group_id='.$group_id.'&atid='.$atid.'">'.
                    $hp->purify($GLOBALS['ath']->getName()).
                '</A>\' -  '.
                $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','modify_chart', array($hp->purify($chart->getTitle())));
        echo ' '.help_button('TrackerAdministration.html#GraphTrackerReportSetting').'</H2>';
        echo '<script type="text/javascript" src="/plugins/graphontrackersv5/dependencies.js"></script>';
        
        $url = '/tracker/admin/?func=reportgraphic&amp;group_id='. $group_id .'&amp;atid='. $atid .'&amp;report_graphic_id='. (int)$chart->getGraphicReport()->getId();
        echo '<p><a href="'. $url .'">&laquo; '. $GLOBALS['Language']->getText('plugin_graphontrackersv5_include_report','return_report').' '. $chart->getgraphicReport()->getName() .'</a></p>';
        echo '<form action="'. $url .'&amp;edit_chart='. $chart->getId() .'" name="edit_chart_form" method="post">';
        echo '<table>';
        echo '<thead><tr class="boxtable"><th class="boxtitle">'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_boxtable','chart_properties').'</th><th class="boxtitle">'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_boxtable','preview').'</th></tr></thead>';
    ...
    function displayReportGraphic($report_graphic_id, $atid, $url){
    ...
    function genGraphRepSelBox($value) {
        $hp =& Codendi_HTMLPurifier::instance();
        require_once(dirname(__FILE__)."/../data-access/GraphOnTrackersV5_Report.class.php");
        $reports  = $this->grf->getReports_ids();
        $returns  = '<form name="plugin_graphontrackersv5_selectreport_form" action="'. $_SERVER['REQUEST_URI'] .'" method="GET">';
        ...
        $returns .= '<B>'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_graphic_report_label','use_graphic_report').'&nbsp;&nbsp;</B>' .
                    '<SELECT NAME="report_graphic_id" onChange="document.plugin_graphontrackersv5_selectreport_form.go_graphreport.click()">';
        $returns .= '<OPTION VALUE="0">'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_empty_select','none_value').'</OPTION>';
        for ($i=0;$i<count($reports);$i++){
            $r = new GraphOnTrackersV5_Report($reports[$i]);
            if ($reports[$i] == $value) {
        ...
        $returns .= '</SELECT>&nbsp;<INPUT TYPE="submit" VALUE="'.$GLOBALS['Language']->getText('plugin_graphontrackersv5_report','btn_go').'" NAME="go_graphreport"/>';
        $returns .= '</form>';
        return $returns;
    }
}
