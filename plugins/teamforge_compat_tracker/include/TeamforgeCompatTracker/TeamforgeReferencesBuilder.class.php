...
use Tuleap\TeamforgeCompatTracker\Reference\ArtifactReference;
use Tuleap\TeamforgeCompatTracker\Reference\TrackerReference;
use Tuleap\TeamforgeCompatTracker\ReferencesImporter;
use Reference;
use ReferenceInstance;
use ProjectManager;

class TeamforgeReferencesBuilder
{
    ...
    public function __construct(TeamforgeCompatDao $dao, ProjectManager $project_manager)
    {
        $this->dao             = $dao;
        ...
        return array(
            array(
                ...
                'regexp' => '/
                    ...
                    (?P<ref>
                        (?P<key>'.ReferencesImporter::TEAMFORGE_XREF_ARTIFACT.'|'.ReferencesImporter::TEAMFORGE_XREF_TRACKER.')
                        (?P<val>[0-9]+)
                    )
    ...
    /**
     * Find a reference given a keyword and the teamforge complete reference
     *
    ...
    private function findReference($keyword, $reference)
    {
        ...
        switch ($keyword) {
            case ReferencesImporter::TEAMFORGE_XREF_TRACKER:
                $reference = new TrackerReference($keyword, $target, $project_id);
                break;
            case ReferencesImporter::TEAMFORGE_XREF_ARTIFACT:
