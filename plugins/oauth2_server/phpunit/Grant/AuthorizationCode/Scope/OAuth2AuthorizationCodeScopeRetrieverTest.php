...
final class OAuth2AuthorizationCodeScopeRetrieverTest extends TestCase
{
    use MockeryPHPUnitIntegration;

    /**
     * @var \Mockery\LegacyMockInterface|\Mockery\MockInterface|OAuth2AuthorizationCodeScopeDAO
     */
    private $scope_dao;
    /**
    ...
    {
        $this->scope_dao     = \Mockery::mock(OAuth2AuthorizationCodeScopeDAO::class);
        $this->scope_builder = \Mockery::mock(AuthenticationScopeBuilder::class);

        $this->retriever = new OAuth2AuthorizationCodeScopeRetriever($this->scope_dao, $this->scope_builder);
    }

    public function testRetrievesScopesAssociatedWithAnAccessToken(): void
    {
        $this->scope_dao->shouldReceive('searchScopeIdentifiersByOAuth2AuthCodeID')->andReturn([
            ['scope_key' => 'profile'],
        ...
        $scopes = $this->retriever->getScopesByAuthorizationCode($this->buildToken());
