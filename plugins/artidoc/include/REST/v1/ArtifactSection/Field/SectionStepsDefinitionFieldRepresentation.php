 ...
 * @psalm-immutable
 */
final readonly class SectionStepsDefinitionFieldRepresentation
{
    public string $type;
    public string $label;
    public string $display_type;
    /**
     * @var list<StepValueRepresentation> $value
     */
    public array $value;

    public function __construct(StepsDefinitionFieldWithValue $field)
    {
        $this->type         = FieldType::STEPS_DEFINITION->value;
        $this->label        = $field->label;
        $this->display_type = $field->display_type->value;
        $this->value        = array_map(
            static fn(StepValue $step) => new StepValueRepresentation($step),
            $field->steps
        );
    }
}
