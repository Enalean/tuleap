...
): VueWrapper<InstanceType<typeof ParentCardRemainingEffort>> {
...
describe("ParentCardRemainingEffort", () => {
    it("displays the remaining effort of the parent card in a badge", () => {
        const wrapper = getWrapper({ value: 666 } as RemainingEffort);
    ...
    it("displays a spinner instead of a flag if remaining effort is being saved", () => {
        const wrapper = getWrapper({ value: 666, is_being_saved: true } as RemainingEffort);
    ...
    it("displays an input instead of the value if the remaining effort is in edit mode", () => {
        const wrapper = getWrapper({ value: 666, is_in_edit_mode: true } as RemainingEffort);

        expect(wrapper.text()).toBe("");
        expect(wrapper.findComponent(EditRemainingEffort).exists()).toBe(true);
    });

    it("is a focusable button if remaining effort can be updated", () => {
    ...
    it("is not a focusable button if remaining effort can not be updated", () => {
        const wrapper = getWrapper({
            value: 666,
            can_update: false,
            is_in_edit_mode: false,
    ...
    it("displays nothing if the parent card has no remaining effort value", () => {
        const wrapper = getWrapper({ value: null } as RemainingEffort);

        expect(wrapper.html()).toBe("<!--v-if-->");
    });

    it("sends a `editor-closed` event when the edition of remaining effort is closed", () => {
        const wrapper = getWrapper({ value: 666, is_in_edit_mode: true } as RemainingEffort);
