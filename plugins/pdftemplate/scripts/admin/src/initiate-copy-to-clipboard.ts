...
export function initiateCopyToClipboard(): void {
    document.querySelectorAll(".pdftemplate-admin-template-copy").forEach((element: Element) => {
        if (!(element instanceof HTMLElement)) {
            return;
        }

        const original = element.dataset.tlpTooltip;
        const icon = element.querySelector(".pdftemplate-admin-template-copy-icon");
