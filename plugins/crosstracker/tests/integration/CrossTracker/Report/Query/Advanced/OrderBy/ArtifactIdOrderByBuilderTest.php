...
final class ArtifactIdOrderByBuilderTest extends CrossTrackerFieldTestCase
{
    ...
    public function testNoOrderBy(): void
    {
        $result = $this->getQueryResults(
            new CrossTrackerQuery($this->uuid, 'SELECT @id FROM @project = "self" WHERE @id >= 1', '', '', 1),
            $this->user,
        );
    ...
    {
        $result = $this->getQueryResults(
            new CrossTrackerQuery($this->uuid, 'SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @id DESC', '', '', 1),
            $this->user,
        );
    ...
    {
        $result = $this->getQueryResults(
            new CrossTrackerQuery($this->uuid, 'SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @id ASC', '', '', 1),
            $this->user,
        );
