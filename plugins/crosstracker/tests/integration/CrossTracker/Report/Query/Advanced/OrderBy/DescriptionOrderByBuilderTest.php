...
final class DescriptionOrderByBuilderTest extends CrossTrackerFieldTestCase
{
    ...
    public function testLastUpdateDateDescending(): void
    {
        $result = $this->getQueryResults(
            new CrossTrackerQuery($this->uuid, 'SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @description DESC', '', '', 1),
            $this->user,
        );
    ...
    {
        $result = $this->getQueryResults(
            new CrossTrackerQuery($this->uuid, 'SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @description ASC', '', '', 1),
            $this->user,
        );
