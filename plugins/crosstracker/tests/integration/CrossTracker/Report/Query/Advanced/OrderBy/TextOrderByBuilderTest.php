...
final class TextOrderByBuilderTest extends CrossTrackerFieldTestCase
{
    ...
    public function testLastUpdateDateDescending(): void
    {
        $result = $this->getQueryResults(
            new CrossTrackerExpertReport(1, 'SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY text_field DESC'),
            $this->user,
        );
    ...
    {
        $result = $this->getQueryResults(
            new CrossTrackerExpertReport(1, 'SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY text_field ASC'),
            $this->user,
        );
