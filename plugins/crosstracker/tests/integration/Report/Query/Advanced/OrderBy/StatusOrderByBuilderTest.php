...
final class StatusOrderByBuilderTest extends CrossTrackerFieldTestCase
{
    ...
    public function testLastUpdateDateDescending(): void
    {
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                 ->withUUID($this->uuid)->withTqlQuery('SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @status DESC')->build(),
            $this->user,
        );
    ...
    public function testLastUpdateDateAscending(): void
    {
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                 ->withUUID($this->uuid)->withTqlQuery('SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @status ASC')->build(),
            $this->user,
        );
