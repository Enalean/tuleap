...
final class SubmittedOnOrderByBuilderTest extends CrossTrackerFieldTestCase
{
    ...
    public function testSubmittedOnDescending(): void
    {
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                 ->withUUID($this->uuid)->withTqlQuery('SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @submitted_on DESC')->build(),
            $this->user,
        );
    ...
    public function testSubmittedOnAscending(): void
    {
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                 ->withUUID($this->uuid)->withTqlQuery('SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @submitted_on ASC')->build(),
            $this->user,
        );
