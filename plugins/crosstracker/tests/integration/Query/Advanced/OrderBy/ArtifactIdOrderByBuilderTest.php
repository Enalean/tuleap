...
final class ArtifactIdOrderByBuilderTest extends CrossTrackerFieldTestCase
{
    ...
    public function testArtifactIdDescending(): void
    {
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                 ->withUUID($this->uuid)->withTqlQuery('SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @id DESC')->build(),
            $this->user,
        );
    ...
    public function testArtifactIdAscending(): void
    {
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                 ->withUUID($this->uuid)->withTqlQuery('SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @id ASC')->build(),
            $this->user,
        );
