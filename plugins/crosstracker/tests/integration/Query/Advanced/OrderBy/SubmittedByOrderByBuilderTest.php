...
final class SubmittedByOrderByBuilderTest extends CrossTrackerFieldTestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $release_artifact_1_id = $tracker_builder->buildArtifact($release_tracker->getId(), 0, (int) $this->user->getId());
        $release_artifact_2_id = $tracker_builder->buildArtifact($release_tracker->getId(), 0, (int) $alice->getId());
        $sprint_artifact_3_id  = $tracker_builder->buildArtifact($sprint_tracker->getId(), 0, (int) $bob->getId());

        $tracker_builder->buildLastChangeset($release_artifact_1_id);
        $tracker_builder->buildLastChangeset($release_artifact_2_id);
        $tracker_builder->buildLastChangeset($sprint_artifact_3_id);
    ...
    public function testSubmittedByDescending(): void
    {
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                 ->withUUID($this->uuid)->withTqlQuery('SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @submitted_by DESC')->build(),
            $this->user,
        );
    ...
    public function testSubmittedByAscending(): void
    {
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                 ->withUUID($this->uuid)->withTqlQuery('SELECT @id FROM @project = "self" WHERE @id >= 1 ORDER BY @submitted_by ASC')->build(),
            $this->user,
        );
