...
final class UnknownSelectBuilderTest extends CrossTrackerFieldTestCase
{
    private UUID $uuid;
    /**
    ...
    {
        //Only the "text_field" column has a known value
        $result = $this->getQueryResults(
            CrossTrackerQueryTestBuilder::aQuery()
                    ->withUUID($this->uuid)->withTqlQuery(
                        "SELECT some_unknown_column FROM @project = 'self' WHERE text_field = '' OR text_field != ''",
                    )->build(),
            $this->user,
        );
        self::assertSame(2, $result->getTotalSize());
        self::assertCount(2, $result->selected);
        self::assertSame('some_unknown_column', $result->selected[1]->name);
        self::assertSame('unknown', $result->selected[1]->type);
        $values = [];
        foreach ($result->artifacts as $artifact) {
            self::assertCount(2, $artifact);
            self::assertArrayHasKey('some_unknown_column', $artifact);
            $value = $artifact['some_unknown_column'];
            self::assertInstanceOf(UnknownTypeResultRepresentation::class, $value);
            $values[] = $value->value;
        }
