...
final class InvalidFromCollectionBuilderTest extends TestCase
{
    ...
    private RetrieveCrossTrackerWidget $project_widget_retriever;
    private RetrieveCrossTrackerWidget $user_widget_retriever;
    ...
    private function getInvalidFrom(
        RetrieveCrossTrackerWidget $widget_retriever,
        From $from,
        ?int $widget_id = 2,
    ): array {
        $in_project_checker = new WidgetInProjectChecker($widget_retriever);
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('blabla', new FromProjectEqual('')), null));
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromTracker('blabla', new FromTrackerEqual('')), null));
        self::assertNotEmpty($result);
        self::assertStringContainsStringIgnoringCase("You cannot search on 'blabla'", $result[0]);
    }
    ...
    {
        $result = $this->getInvalidFrom($this->user_widget_retriever, new From(
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(
            new FromProject('@project', new FromProjectEqual('self')),
            new FromTracker('@tracker.name', new FromTrackerEqual('release')),
        ));
    ...
    {
        $result = $this->getInvalidFrom($this->user_widget_retriever, new From(new FromProject('@project', new FromProjectIn(['self'])), null));
    ...
    {
        $result = $this->getInvalidFrom($this->user_widget_retriever, new From(new FromProject('@project', new FromProjectIn(['aggregated'])), null));
        self::assertCount(1, $result);
    ...
    public function testItReturnsErrorWhenProjectInAggregatedInsideNormalProject(): void
    {
        ...
        $result                = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project', new FromProjectIn(['self', 'aggregated'])), null));
        self::assertCount(1, $result);
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project', new FromProjectIn(['self'])), null));
        self::assertEmpty($result);
    }

    public function testItReturnsErrorWhenProjectSelfOutsideProject(): void
    {
        $result = $this->getInvalidFrom($this->user_widget_retriever, new From(new FromProject('@project', new FromProjectEqual('self')), null));
        self::assertCount(1, $result);
        self::assertEquals("You cannot use @project with 'self' in the context of a personal dashboard", $result[0]);
    }
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project', new FromProjectEqual('self')), null));
        self::assertEmpty($result);
    }

    public function testItReturnsErrorWhenProjectAggregatedOutsideProject(): void
    {
        $result = $this->getInvalidFrom($this->user_widget_retriever, new From(new FromProject('@project', new FromProjectEqual('aggregated')), null));
        self::assertCount(1, $result);
        self::assertEquals("You cannot use @project with 'aggregated' in the context of a personal dashboard", $result[0]);
    }
    ...
    {
        $this->project_factory = ProjectByIDFactoryStub::buildWith(ProjectTestBuilder::aProject()->withId(101)->build());
        $result                = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project', new FromProjectEqual('aggregated')), null));
        self::assertCount(1, $result);
        self::assertEquals("You cannot use @project with 'aggregated' in a project without service Program enabled", $result[0]);
    }
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project.name', new FromProjectEqual('')), null));
        self::assertCount(1, $result);
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project.name', new FromProjectIn(['My project', ''])), null));
        self::assertCount(1, $result);
        self::assertEquals('@project.name cannot be empty', $result[0]);
    }
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project.name', new FromProjectEqual('Project name')), null));
        self::assertEmpty($result);

        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project.name', new FromProjectIn(['My project', 'Awesome project'])), null));
        self::assertEmpty($result);
    }

    public function testItReturnsErrorWhenProjectCategoryEqualEmpty(): void
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project.category', new FromProjectEqual('')), null));
        self::assertCount(1, $result);
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project.category', new FromProjectIn(['some', '', 'empty'])), null));
        self::assertCount(1, $result);
        self::assertEquals('@project.category cannot be empty', $result[0]);
    }
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project.category', new FromProjectEqual('some')), null));
        self::assertEmpty($result);

        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromProject('@project.category', new FromProjectIn(['some', 'value'])), null));
        self::assertEmpty($result);
    }

    public function testItReturnsErrorWhenTrackerNameOutsideProjectWithoutProjectCondition(): void
    {
        $result = $this->getInvalidFrom($this->user_widget_retriever, new From(new FromTracker('@tracker.name', new FromTrackerEqual('release')), null));
        self::assertCount(1, $result);
        ...
        $result = $this->getInvalidFrom($this->user_widget_retriever, new From(new FromTracker('@tracker.name', new FromTrackerIn(['release'])), null));
        self::assertCount(1, $result);
        self::assertEquals('In the context of a personal dashboard, you must provide a @project condition in the FROM part of your query', $result[0]);

        $result = $this->getInvalidFrom($this->user_widget_retriever, new From(new FromTracker('@tracker.name', new FromTrackerIn(['release'])), null), null);
        self::assertCount(1, $result);
    ...
    {
        self::assertEmpty($this->getInvalidFrom($this->project_widget_retriever, new From(
            new FromTracker('@tracker.name', new FromTrackerEqual('release')),
    ...
    {
        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromTracker('@tracker.name', new FromTrackerEqual('')), null));
        self::assertCount(1, $result);
        self::assertEquals('@tracker.name cannot be empty', $result[0]);

        $result = $this->getInvalidFrom($this->project_widget_retriever, new From(new FromTracker('@tracker.name', new FromTrackerIn(['release', '', 'sprint'])), null));
        self::assertCount(1, $result);
        self::assertEquals('@tracker.name cannot be empty', $result[0]);
    }
}
