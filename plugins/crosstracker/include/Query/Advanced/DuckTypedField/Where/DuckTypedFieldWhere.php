 ...
 * @psalm-immutable
 */
final readonly class DuckTypedFieldWhere
{
    /**
    ...
    public static function build(
        ...
        foreach ($fields as $field) {
            $field_identifiers[] = DuckTypedFieldTypeWhere::fromString($retrieve_field_type->getType($field))
                ->map(static fn(DuckTypedFieldTypeWhere $type) => new FieldIdentifierPropertiesWhere($field->getId(), $type));
        }

        $other_results = array_slice($field_identifiers, 1);

        return $field_identifiers[0]->andThen(
            static function (FieldIdentifierPropertiesWhere $first_field) use ($field_name, $tracker_ids, $other_results) {
