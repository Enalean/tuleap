	...
	var tabString=
		"<thead>" +
			"<tr class=\"boxtable\">" +
				...
				"<th class=\"boxtitle\" scope=\"col\">Versions</th>" +
			"</tr>" +
		"</thead>"
	;
	...
	function retrieveAllFiles(callback) 
	{
		...
		gapi.client.load('drive', 'v2', function() 
		{
			/*var initialRequest = gapi.client.drive.files.list({
				'folderId': '0BxQ-D1tzBR64anRJLWliV0dseTg'
			});*/
	...
	function printFile(fileId) 
	{
		gapi.client.load('drive', 'v2', function() 
		{
			...
			request.execute(function(resp) 
			{
				...
				requestRev.execute(function(resp2) 
				{
					if(resp2.kind != null)
					{
						var nb=0;

						document.getElementById("revisions"+fileId).innerHTML+="<select id=\"select"+fileId
						+"\" name=\""+fileId+"\" onChange=\";document.getElementById('a"+fileId+"').href=document.getElementById('select"+fileId+"').options[document.getElementById('select"+fileId+"').selectedIndex].value;\">";
						while(nb != 5)
						{
							if(resp2.items.length-1-nb < 0)
								break;

							if(resp2.items[resp2.items.length-1-nb].exportLinks != null)
							{
								var dateCreation=resp2.items[resp2.items.length-1-nb].modifiedDate.substr(8,2)+"/"+resp2.items[resp2.items.length-1-nb].modifiedDate.substr(5,2)+"/"+resp2.items[resp2.items.length-1-nb].modifiedDate.substr(0,4);
								document.getElementsByName(fileId)[0].innerHTML+="<option value=\""+resp2.items[resp2.items.length-1-nb].exportLinks["application/pdf"]+"\">"+dateCreation+"</option>";
							}

							nb++;
						}
						document.getElementById('revisions'+fileId).innerHTML+="</select>";
